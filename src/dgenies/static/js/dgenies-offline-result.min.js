...
d3.dgenies.svgcontainer=null;d3.dgenies.container=null;d3.dgenies.top_track_container=null;d3.dgenies.right_track_container=null;d3.dgenies.svgsupercontainer=null;d3.dgenies.name_x=null;d3.dgenies.name_y=null;d3.dgenies.lines=null;d3.dgenies.x_len=null;d3.dgenies.y_len=null;d3.dgenies.x_zones=null;d3.dgenies.y_zones=null;d3.dgenies.x_track=null;d3.dgenies.y_track=null;d3.dgenies.zoom_enabled=true;d3.dgenies.all_disabled=false;d3.dgenies.min_idy=0;d3.dgenies.max_idy=0;d3.dgenies.zone_selected=false;d3.dgenies.query_selected=null;//For translations:
...
d3.dgenies.xpickr=null;d3.dgenies.ypickr=null;/**
 * Initialize dotplot
 *
 * @param {string} id_res job id
 ...
 */d3.dgenies.draw_background=function(x_order,y_order){let x_zones=d3.dgenies.x_zones,y_zones=d3.dgenies.y_zones;console.log(x_zones,y_zones);for(let i=0;i<x_order.length;i++){let x=x_order[i];let color_shift=i%d3.dgenies.color_background.length;for(let j=0;j<y_order.length;j++){let y=y_order[j];let color_idx=(color_shift+j)%d3.dgenies.color_background.length;d3.dgenies.container.append("rect").attr("x",x_zones[x][0]).attr("y",d3.dgenies.scale-y_zones[y][1]).attr("width",x_zones[x][1]-x_zones[x][0]).attr("height",y_zones[y][1]-y_zones[y][0]).attr("stroke","none").attr("fill",d3.dgenies.color_background[color_idx])}}};/**
...
$("path.content-lines").remove();for(let i=0;i<4;i++){d3.dgenies.__draw_idy_lines(i.toString(),lines,x_len,y_len)}d3.dgenies.events.filter_size(d3.dgenies.min_size)};d3.dgenies.create_pickr=function(el,default_color="#FFFFFF00"){return new Pickr({el:el,default:default_color,theme:"nano",lockOpacity:false,swatches:["rgba(244, 67, 54, 1)","rgba(233, 30, 99, 0.95)","rgba(156, 39, 176, 0.9)","rgba(103, 58, 183, 0.85)","rgba(63, 81, 181, 0.8)","rgba(33, 150, 243, 0.75)","rgba(3, 169, 244, 0.7)","rgba(0, 188, 212, 0.7)","rgba(0, 150, 136, 0.75)","rgba(76, 175, 80, 0.8)","rgba(139, 195, 74, 0.85)","rgba(205, 220, 57, 0.9)","rgba(255, 235, 59, 0.95)","rgba(255, 193, 7, 1)"],components:{preview:true,opacity:true,hue:true,interaction:{hex:true,rgba:true,hsva:true,input:true,clear:true,save:true}}})};d3.dgenies.add_restore_all=function(){let draw=$("#draw");draw.append($("<div>").attr("class","restore-all").html(`
                ...
                `));d3.dgenies.xpickr=d3.dgenies.create_pickr("#xpickr");$("#rowColor button").addClass("btn btn-sm").attr("id","xpickr").after(`<label for="xpickr">Row</label>`);d3.dgenies.ypickr=d3.dgenies.create_pickr("#ypickr");$("#columnColor button").addClass("btn btn-sm").attr("id","ypickr").after(`<label for="ypickr">Column</label>`)};d3.dgenies.add_restore_all2=function(){let draw=$("#draw");draw.append($("<div>").attr("class","restore-all").html(`
