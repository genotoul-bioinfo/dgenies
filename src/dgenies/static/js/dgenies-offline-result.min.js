 ...
 */dgenies.init=function(all_jobs,mode){dgenies.mode=mode;let cookies=$.cookie("results");let cookie_wall=$.cookie("wall");if(mode==="webserver"){if(cookie_wall===undefined){$("#cwall").modal({escapeClose:false,clickClose:false,showClose:false})}cookies=cookies!==undefined&&cookies.length>0?cookies.split("|"):[]}else{cookies=all_jobs;dgenies.save_cookies(cookies)}dgenies.update_results(cookies)};/**
 * Save cookie on the browser
 * @param {array} cookies list of jobs
 */dgenies.save_cookies=function(cookies){$.cookie("results",cookies.join("|"),{path:"/"})};dgenies.accept_cookie_wall=function(){$.cookie("wall","",{expires:180,path:"/"})};/**
 * Update list of jobs
 * @param {array} results: new list of jobs
 */dgenies.update_results=function(results){let job_list_item=$("ul.nav li.result ul");job_list_item.html("");if(results.length>0){for(let i=0;i<results.length;i++){let result=results[i];job_list_item.append($("<li>").append($("<a>").addClass("dropdown-item").attr("href",`/result/${result}`).text(result)))}}else{job_list_item.append($("<li>").append($("<a>").addClass("dropdown-item").attr("href","/run").text("Please run a job!")))}};/**
 ...
 */dgenies.notify=function(text,type="warning",delay=5000){$.notify({message:text},{type:type,placement:{from:"top",align:"center"},delay:delay,animate:{enter:"animated fadeInDown",exit:"animated fadeOutUp"},offset:55,newest_on_top:true})};/**
 * Show loading popup
 *
 * @param {string} message loading message
 * @param {int} width popup width
 */dgenies.show_loading=function(message="Loading...",width=118){$("input,form#export select").prop("disabled",true);d3.dgenies.all_disabled=true;$(dgenies.loading).find(".mylabel").html(message);$(dgenies.loading).find(".label").width(width);$(dgenies.loading).show();$(dgenies.loading).position({my:"center center",at:"center center",of:"#draw",collistion:"fit"})};/**
 * Hide loading popup
 */dgenies.hide_loading=function(){$("input,form#export select").prop("disabled",false);d3.dgenies.all_disabled=false;$(dgenies.loading).hide();dgenies.reset_loading_message()};/**
 * Change loading message on current popup
 *
 * @param {string} message new message
 */dgenies.set_loading_message=function(message){$(dgenies.loading).find(".mylabel").html(message)};/**
 * Reset loading message to its default value
 */dgenies.reset_loading_message=function(){$(dgenies.loading).find(".mylabel").html("Loading...");$(dgenies.loading).find(".label").width(118)};/**
 ...
 */dgenies.post=function(url,data,success,error,async=true){dgenies.ajax({url:url,data:data,success:success,error:error,type:"POST",async:async})};/**
 * Get server call
 *
 * @param url url to call
 * @param data data to send
 ...
 */dgenies.result.remove_job_from_cookie=function(job){let cookies=$.cookie("results");cookies=cookies!==undefined?cookies.split("|"):[];let index=cookies.indexOf(job);let need_update=false;if(index>-1){need_update=true;cookies.splice(index,1)}$.cookie("results",cookies.join("|"),{path:"/"});if(need_update){dgenies.update_results(cookies)}};if(!dgenies||!dgenies.result){throw"dgenies.result wasn't included!"}dgenies.result.controls={};/**
 ...
 */dgenies.result.summary.export_png=function(){let export_div=$("div#export-pict");export_div.html("").append($("<canvas>"));canvg(export_div.find("canvas")[0],dgenies.result.summary.get_svg());let canvas=export_div.find("canvas")[0];canvas.toBlob(function(blob){dgenies.result.summary.save_file(blob,"png");export_div.html("")},"image/png")};if(!d3){throw"d3 wasn't included!"}d3.dgenies={};//GLOBAL VARIABLES:
d3.dgenies.svgcontainer=null;d3.dgenies.container=null;d3.dgenies.background=null;d3.dgenies.top_track_container=null;d3.dgenies.right_track_container=null;d3.dgenies.svgsupercontainer=null;d3.dgenies.name_x=null;d3.dgenies.name_y=null;d3.dgenies.lines=null;d3.dgenies.x_len=null;d3.dgenies.y_len=null;d3.dgenies.x_zones=null;d3.dgenies.y_zones=null;d3.dgenies.x_track=null;d3.dgenies.y_track=null;d3.dgenies.x_order=null;d3.dgenies.y_order=null;d3.dgenies.zoom_enabled=true;d3.dgenies.all_disabled=false;d3.dgenies.min_idy=0;d3.dgenies.max_idy=0;d3.dgenies.zone_selected=false;d3.dgenies.query_selected=null;d3.dgenies.are_hidden_tracks=[false,false];//For translations:
...
d3.dgenies.scale=1000;d3.dgenies.axis_length=500;d3.dgenies.content_lines_width=d3.dgenies.scale/400;d3.dgenies.break_lines_width=d3.dgenies.scale/1500;/**
 ...
 */d3.dgenies.hex_to_greyscale=function(hex){// Convert hex to RGB first
let r=0,g=0,b=0;if(hex.length==4){r="0x"+hex[1]+hex[1];g="0x"+hex[2]+hex[2];b="0x"+hex[3]+hex[3]}else if(hex.length==7){r="0x"+hex[1]+hex[2];g="0x"+hex[3]+hex[4];b="0x"+hex[5]+hex[6]}// Then to HSL
r/=255;g/=255;b/=255;let cmin=Math.min(r,g,b),cmax=Math.max(r,g,b),delta=cmax-cmin,h=0,s=0,l=0;if(delta==0)h=0;else if(cmax==r)h=(g-b)/delta%6;else if(cmax==g)h=(b-r)/delta+2;else h=(r-g)/delta+4;h=Math.round(h*60);if(h<0)h+=360;l=(cmax+cmin)/2;//s = delta == 0 ? 0 : delta / (1 - Math.abs(2 * l - 1));
//s = +(s * 100).toFixed(1);
l=+(l*100).toFixed(1);return"hsl("+0+","+0+"%,"+l+"%)"};//Color theme parameters:
d3.dgenies.color_idy_theme="default";d3.dgenies.color_idy_themes=["default","colorblind","greyscale","r_default","r_colorblind","allblack"];d3.dgenies.color_idy={"default":{"palette":{"3":"#094b09","2":"#2ebd40","1":"#d5670b","0":"#ffd84b","-1":"#fff"},"track_color":val=>{return val},"track_heatmap":d3.interpolateReds},"colorblind":{"palette":{"3":"#000","2":"#006DDB","1":"#DB6E00","0":"#FFB677","-1":"#fff"},"track_color":val=>{return val},"track_heatmap":d3.interpolateViridis},"greyscale":{"palette":{"3":"#000","2":"#626262","1":"#9c9c9c","0":"#DDDCDC","-1":"#fff"},"track_color":d3.dgenies.hex_to_greyscale,"track_heatmap":d3.interpolateGreys},"r_default":{"palette":{"3":"#7fff65","2":"#238d31","1":"#78410d","0":"#3b080a","-1":"#fff"},"track_color":val=>{return val},"track_heatmap":d3.interpolateBlues},"r_colorblind":{"palette":{"3":"#8c8c8c","2":"#006DDB","1":"#783c00","0":"#312515","-1":"#fff"},"track_color":val=>{return val},"track_heatmap":d3.interpolateViridis},"allblack":{"palette":{"3":"#000","2":"#000","1":"#000","0":"#000","-1":"#fff"},"track_color":_=>{return"#000000"},"track_heatmap":t=>{return"#000000"}}};d3.dgenies.track_default_color="#0000ff";d3.dgenies.right_track_color=null;d3.dgenies.top_track_color=null;d3.dgenies.limit_idy=null;d3.dgenies.min_idy_draw=0;d3.dgenies.min_size=0;d3.dgenies.linecap="round";d3.dgenies.background_axis="#f4f4f4";d3.dgenies.break_lines_color="#7c7c7c";d3.dgenies.break_lines_dash="3, 3";d3.dgenies.break_lines_show=true;d3.dgenies.zoom_scale_lines=1;// Zoom scale used for lines width
d3.dgenies.tick_width=0.5;d3.dgenies.color_mixes="#969696";//Container sizes (relative ones on a 100 scale)
d3.dgenies.container_axis_thickness=5;d3.dgenies.container_track_thickness=3;d3.dgenies.container_dotplot_base=100;d3.dgenies.container_dotplot_width=d3.dgenies.container_dotplot_base-d3.dgenies.container_axis_thickness-d3.dgenies.container_track_thickness;d3.dgenies.container_dotplot_height=d3.dgenies.container_dotplot_width;//Filter sizes:
d3.dgenies.min_sizes=[0,0.01,0.02,0.03,0.05,1,2];//Help pictures:
d3.dgenies.help_zoom="/static/images/ctrl_plus_mouse.png";d3.dgenies.help_trans="/static/images/ctrl_plus_click.png";//Color pickers
d3.dgenies.x_pickr=null;d3.dgenies.y_pickr=null;d3.dgenies.x_background_color="rgba(255,255,255,0)";d3.dgenies.y_background_color="rgba(255,255,255,0)";/**
 * Initialize dotplot
 *
 * @param {string} id_res job id
 ...
 */d3.dgenies.launch=function(res,update=false,noise_change=false){d3.dgenies.x_order=res["x_order"];d3.dgenies.y_order=res["y_order"];dgenies.fill_select_zones(d3.dgenies.x_order,d3.dgenies.y_order);if(res["sorted"]){$("button#sort-contigs").text("Undo sort");$("#export").find("select option[value=4]").show();$("#export").find("select option[value=8]").show()}else{$("button#sort-contigs").text("Sort contigs");$("#export").find("select option[value=4]").hide();$("#export").find("select option[value=8]").hide()}d3.dgenies.name_x=res["name_x"];d3.dgenies.name_y=res["name_y"];if(d3.dgenies.name_x===d3.dgenies.name_y){$("button#sort-contigs").parent().hide()}else{$("button#sort-contigs").parent().show()}d3.dgenies.lines=res["lines"];d3.dgenies.x_len=res["x_len"];d3.dgenies.y_len=res["y_len"];d3.dgenies.min_idy=res["min_idy"];d3.dgenies.max_idy=res["max_idy"];d3.dgenies.limit_idy=res["limit_idy"];if(res["x_track"]!=undefined){d3.dgenies.x_track=res["x_track"]}if(res["y_track"]!=undefined){d3.dgenies.y_track=res["y_track"]}if(!noise_change){dgenies.noise=true}$("#hide-noise").val(dgenies.noise?"Hide noise":"Show noise");d3.dgenies.draw(res["x_contigs"],d3.dgenies.x_order,res["y_contigs"],d3.dgenies.y_order);if(!update){$("div#draw").resizable({aspectRatio:true,maxWidth:$("#body-container").width()-parseFloat($("#sidebar").css("min-width"))});d3.dgenies.events.init();dgenies.result.controls.init()}if(res["sampled"]){let max_nb_lines=dgenies.numberWithCommas(res["max_nb_lines"].toString());dgenies.notify(`<div style="text-align: center"><b>There are too much matches.\nOnly the ${max_nb_lines} best matches are displayed</b></div>`)}d3.dgenies.mousetip.init()};/**
...
let pseudo_x_zones={};pseudo_x_zones[x_zone]=[0,d3.dgenies.x_len];d3.dgenies.draw_top_axis(pseudo_x_zones);let pseudo_y_zones={};pseudo_y_zones[y_zone]=[0,d3.dgenies.y_len];d3.dgenies.draw_right_axis(pseudo_y_zones);//Update tracks:
d3.dgenies.zoom_top_track();d3.dgenies.zoom_right_track();d3.dgenies.zoom_enabled=false}$("#restore-all").prop("disabled",false);dgenies.hide_loading()},0)};/**
 ...
 */d3.dgenies.draw_left_axis=function(y_max,y_min=0){let axis_length=d3.dgenies.axis_length;$("svg.left-axis").remove();//Remove previous axis (if any)
let svg_left=d3.dgenies.svgsupercontainer.append("svg:svg").attr("class","axis left-axis").attr("width",d3.dgenies.container_axis_thickness).attr("height",d3.dgenies.container_dotplot_height-d3.dgenies.container_axis_thickness).attr("x",0).attr("y",d3.dgenies.container_axis_thickness+d3.dgenies.container_track_thickness).attr("viewBox","0 0 20 "+axis_length).attr("preserveAspectRatio","none");let container_left=svg_left.append("g").attr("transform","translate(0,"+axis_length+")rotate(-90)");let y_size=y_max-y_min;for(let i=1;i<10;i++){let y=axis_length/10*i;let y_t=y_min+y_size/10*i;if(y_t>=0&&y_t<=d3.dgenies.y_len){let y_lab=d3.dgenies.get_human_readable_size(y_t);container_left.append("line").attr("x1",y).attr("y1",15).attr("x2",y).attr("y2",20).attr("stroke-width",d3.dgenies.tick_width).attr("stroke","black");container_left.append("text").attr("x",y).attr("y",12).attr("text-anchor","middle").attr("font-family","sans-serif").attr("font-size","6.5pt").text(y_lab)}}};/**
 * Draw bottom axis
 *
 * @param {int} x_max max value of x on the X axis
 * @param {int} x_min min value of x on the X axis
 */d3.dgenies.draw_bottom_axis=function(x_max,x_min=0){let axis_length=d3.dgenies.axis_length;$("svg.bottom-axis").remove();//Remove previous axis (if any)
let svg_bottom=d3.dgenies.svgsupercontainer.append("svg:svg").attr("class","axis bottom-axis").attr("width",d3.dgenies.container_dotplot_width-d3.dgenies.container_axis_thickness).attr("height",d3.dgenies.container_axis_thickness).attr("x",d3.dgenies.container_axis_thickness).attr("y",d3.dgenies.container_dotplot_height+d3.dgenies.container_track_thickness).attr("viewBox","0 0 "+axis_length+" 20").attr("preserveAspectRatio","none");let x_size=x_max-x_min;for(let i=1;i<10;i++){let x=axis_length/10*i;let x_t=x_min+x_size/10*i;if(x_t>=0&&x_t<=d3.dgenies.x_len){let x_lab=d3.dgenies.get_human_readable_size(x_t);svg_bottom.append("line").attr("x1",x).attr("y1",0).attr("x2",x).attr("y2",5).attr("stroke-width",d3.dgenies.tick_width).attr("stroke","black");svg_bottom.append("text").attr("x",x).attr("y",15).attr("text-anchor","middle").attr("font-family","sans-serif").attr("font-size","6.5pt").text(x_lab)}}};/**
 * Zoom on left axis
...
let transform=d3.dgenies.container.attr("transform");if(transform===null){transform="translate(0,0)scale(1)"}let tr_regex=/translate\(([^,]+),([^)]+)\)/;let translate=parseFloat(transform.match(tr_regex)[2]);let sc_regex=/scale\(([^,)]+)(,([^)]+))?\)/;let scale=parseFloat(transform.match(sc_regex)[3]!==undefined?transform.match(sc_regex)[3]:transform.match(sc_regex)[1]);let axis_length=d3.dgenies.axis_length;let svg_right=d3.dgenies.svgsupercontainer.append("svg:svg").attr("class","axis right-axis").attr("width",d3.dgenies.container_axis_thickness).attr("height",d3.dgenies.container_dotplot_height-d3.dgenies.container_axis_thickness).attr("x",d3.dgenies.container_dotplot_width+d3.dgenies.container_track_thickness).attr("y",d3.dgenies.container_axis_thickness+d3.dgenies.container_track_thickness).attr("viewBox","0 0 20 "+axis_length).attr("preserveAspectRatio","none");let container_right=svg_right.append("g").attr("transform","translate(20)rotate(90)");container_right.append("text").attr("x",axis_length/2).attr("y",7.5).attr("font-size","6pt").attr("font-family","sans-serif").attr("font-style","italic").attr("text-anchor","middle").text(d3.dgenies.name_y);let nb_zone=Object.keys(y_zones).length-1;for(let zone in y_zones){let y_pos_2=Math.min(Math.max((d3.dgenies.scale-y_zones[zone][0])*scale+translate,0),d3.dgenies.scale);let y_pos_1=Math.min(Math.max((d3.dgenies.scale-y_zones[zone][1])*scale+translate,0),d3.dgenies.scale);let z_len=y_pos_2/d3.dgenies.scale*axis_length-y_pos_1/d3.dgenies.scale*axis_length;if(!zone.startsWith("###MIX###")){//z_middle = (x_zones[zone][1] + x_zones[zone][0]) / 2
...
container_right.append("line").attr("x1",y_pos_1/d3.dgenies.scale*axis_length).attr("x2",y_pos_1/d3.dgenies.scale*axis_length).attr("y1",12).attr("y2",20).attr("stroke","black").attr("stroke-width",d3.dgenies.tick_width).attr("class","whereis")}nb_zone--}};d3.dgenies.display_track_container=function(){// we display tracks
$("svg.svgcontainer").attr("width",d3.dgenies.container_dotplot_width-d3.dgenies.container_axis_thickness).attr("height",d3.dgenies.container_dotplot_height-d3.dgenies.container_axis_thickness).attr("x",d3.dgenies.container_axis_thickness).attr("y",d3.dgenies.container_axis_thickness+d3.dgenies.container_track_thickness);/*
    $("svg.left-axis")
    ...
    */$("svg.top-track").attr("height",d3.dgenies.container_track_thickness);$("svg.right-track").attr("width",d3.dgenies.container_axis_thickness+d3.dgenies.container_track_thickness)};/**
 * Toggle display top track
 */d3.dgenies.toggle_top_track=function(){if($("#targetTrackSwitch").is(":checked")){// display top track
$("svg.top-track").removeAttr("visibility");if(!$("#queryTrackSwitch").is(":checked")){// display track container
d3.dgenies.display_track_container()}}else{if(!$("#queryTrackSwitch").is(":checked")){// hide track container
d3.dgenies.hide_track_container()}// hide top track
$("svg.top-track").attr("visibility","hidden")}//else nothing to do
};/**
 * Toggle display right track
 */d3.dgenies.toggle_right_track=function(){if($("#queryTrackSwitch").is(":checked")){// display right track
$("svg.right-track").removeAttr("visibility");if(!$("#targetTrackSwitch").is(":checked")){// display track container
d3.dgenies.display_track_container()}}else{if(!$("#targetTrackSwitch").is(":checked")){// hide track container
d3.dgenies.hide_track_container()}// hide right track
$("svg.right-track").attr("visibility","hidden")}};d3.dgenies.hide_track_container=function(){console.log(d3.dgenies.container_dotplot_base,d3.dgenies.container_axis_thickness,d3.dgenies.container_track_thickness);// we hide tracks
$("svg.svgcontainer").attr("width",d3.dgenies.container_dotplot_width-d3.dgenies.container_axis_thickness+d3.dgenies.container_track_thickness).attr("height",d3.dgenies.container_dotplot_height-d3.dgenies.container_axis_thickness+d3.dgenies.container_track_thickness).attr("x",d3.dgenies.container_axis_thickness).attr("y",d3.dgenies.container_axis_thickness);/*
    $("svg.left-axis")
        .attr("height", d3.dgenies.container_dotplot_height - d3.dgenies.container_axis_thickness)
    ...
    */$("svg.top-track").attr("height",0);$("svg.right-track").attr("width",0)};/**
 * Draw top track
 */d3.dgenies.draw_top_track=function(){if(d3.dgenies.x_track!=null){d3.dgenies.set_top_track_color_tmpl();let x_track=d3.dgenies.x_track.data;for(let x_id in x_track){for(let feat of x_track[x_id]){let f_start=feat[0];let f_len=feat[1];let f_val=feat[2];let f_comment=feat[3];let start=f_start/d3.dgenies.x_len*d3.dgenies.scale;length=f_len/d3.dgenies.x_len*d3.dgenies.scale;let block=d3.dgenies.top_track_container.append("rect").attr("x",start).attr("y",0).attr("width",length).attr("height","100%").attr("stroke","none").attr("class","track").attr("fill",d3.dgenies.top_track_color(d3.dgenies.set_track_color(f_val))).attr("fill_origin",f_val)}}}};/**
 * Draw right track
 */d3.dgenies.draw_right_track=function(){if(d3.dgenies.y_track!=null){d3.dgenies.set_right_track_color_tmpl();let y_track=d3.dgenies.y_track.data;for(let y_id in y_track){for(let feat of y_track[y_id]){let f_start=feat[0];let f_len=feat[1];let f_val=feat[2];let f_comment=feat[3];let start=(d3.dgenies.y_len-f_start-f_len)/d3.dgenies.y_len*d3.dgenies.scale;let length=f_len/d3.dgenies.y_len*d3.dgenies.scale;let block=d3.dgenies.right_track_container.append("rect").attr("x",0).attr("y",start).attr("width","100%").attr("height",length).attr("stroke","none").attr("class","track").attr("fill",d3.dgenies.right_track_color(d3.dgenies.set_track_color(f_val))).attr("fill_origin",f_val)}}}};/**
 * Zoom top track
 * 
 * @param {object} scale_correction correction to apply to translate when zooming with the scroll
 */d3.dgenies.zoom_top_track=function(scale_correction=1){let transform=d3.dgenies.container.attr("transform");if(transform===null){transform="translate(0,0)scale(1)"}let tr_regex=/translate\(([^,]+),([^)]+)\)/;let translate_x=parseFloat(transform.match(tr_regex)[1]);let sc_regex=/scale\(([^,)]+)(,([^)]+))?\)/;let scale_x=parseFloat(transform.match(sc_regex)[1]);d3.dgenies.top_track_container.attr("transform","scale("+scale_x+", 1)translate("+translate_x/scale_correction+",0)")};/**
 * Zoom right track
 * @param {object} scale_correction correction to apply to translate when zooming with the scroll
 */d3.dgenies.zoom_right_track=function(scale_correction=1){let transform=d3.dgenies.container.attr("transform");if(transform===null){transform="translate(0,0)scale(1)"}let tr_regex=/translate\(([^,]+),([^)]+)\)/;let translate_y=parseFloat(transform.match(tr_regex)[2]);let sc_regex=/scale\(([^,)]+)(,([^)]+))?\)/;let scale_y=parseFloat(transform.match(sc_regex)[3]!==undefined?transform.match(sc_regex)[3]:transform.match(sc_regex)[1]);d3.dgenies.right_track_container.attr("transform","scale(1,"+scale_y+")translate(0,"+translate_y/scale_correction+")")};/**
 * Draw vertical lines separation contigs
 * @param {*} x_order order of query contigs
 */d3.dgenies.draw_x_lines=function(x_order){let x_zones=d3.dgenies.x_zones;for(let x of x_order){d3.dgenies.container.append("line").attr("x1",x_zones[x][0]).attr("y1",d3.dgenies.scale).attr("x2",x_zones[x][0]).attr("y2",0).attr("class","break-lines").attr("stroke-width",d3.dgenies.break_lines_width).attr("stroke",d3.dgenies.break_lines_color).style("stroke-dasharray",d3.dgenies.break_lines_dash)}};/**
 * Draw horizontal lines separation contigs
 * @param {*} y_order order of target contigs
 */d3.dgenies.draw_y_lines=function(y_order){let y_zones=d3.dgenies.y_zones;for(let y of y_order){d3.dgenies.container.append("line").attr("x1",0).attr("y1",d3.dgenies.scale-y_zones[y][1]).attr("x2",d3.dgenies.scale).attr("y2",d3.dgenies.scale-y_zones[y][1]).attr("class","break-lines").attr("stroke-width",d3.dgenies.break_lines_width).attr("stroke",d3.dgenies.break_lines_color).style("stroke-dasharray",d3.dgenies.break_lines_dash)}};/**
 ...
 */d3.dgenies.draw_background=function(x_order,y_order){let x_zones=d3.dgenies.x_zones,y_zones=d3.dgenies.y_zones;const x_background=[d3.dgenies.x_background_color,"rgba(255,255,255,0)"];// d3.dgenies.x_pickr.getColor().toRGBA().toString()
const y_background=["rgba(255,255,255,0)",d3.dgenies.y_background_color];$("#dotplot-background > rect").remove();for(let i=0;i<x_order.length;i++){let x=x_order[i];let color_idx=i%x_background.length;d3.dgenies.background.append("rect").attr("x",x_zones[x][0]).attr("y",0).attr("width",x_zones[x][1]-x_zones[x][0]).attr("height",d3.dgenies.scale).attr("stroke","none").attr("fill",x_background[color_idx])}for(let j=0;j<y_order.length;j++){let y=y_order[j];let color_idx=j%y_background.length;d3.dgenies.background.append("rect").attr("x",0).attr("y",d3.dgenies.scale-y_zones[y][1]).attr("width",d3.dgenies.scale).attr("height",y_zones[y][1]-y_zones[y][0]).attr("stroke","none").attr("fill",y_background[color_idx])}};/**
 ...
 * @private
 */d3.dgenies.__lineFunction=function(d,min_size=0,max_size=null,x_len,y_len){d=d.sort(d3.dgenies._sort_lines_by_idy);let path=[];for(let d_i of d){let x1=d_i[0]/x_len*d3.dgenies.scale;let x2=d_i[1]/x_len*d3.dgenies.scale;let y1=d3.dgenies.scale-d_i[2]/y_len*d3.dgenies.scale;let y2=d3.dgenies.scale-d_i[3]/y_len*d3.dgenies.scale;let idy=d_i[4];let len=Math.sqrt(Math.pow(x2-x1,2)+Math.pow(y2-y1,2));if(len>min_size&&(max_size===null||len<max_size)&&Math.abs(idy)>=d3.dgenies.min_idy_draw){path.push(`M${x1} ${y1} L${x2} ${y2}`)}}return path.join(" ")};/**
 ...
 * @private
 */d3.dgenies.__draw_idy_lines=function(idy,lines,x_len,y_len){let min_sizes=d3.dgenies.min_sizes;for(let i=0;i<min_sizes.length;i++){let min_size=min_sizes[i];let max_size=i+1<min_sizes.length?min_sizes[i+1]:null;if(lines[idy].length>0){d3.dgenies.container.append("path").attr("d",d3.dgenies.__lineFunction(lines[idy],min_size,max_size,x_len,y_len)).attr("class","content-lines s_"+min_size.toString().replace(".","_")+" idy_"+idy).attr("stroke-width",d3.dgenies.content_lines_width+"px").attr("stroke",d3.dgenies.color_idy[d3.dgenies.color_idy_theme]["palette"][idy]).attr("stroke-linecap",d3.dgenies.linecap)}}};/**
 * Switch to next color theme
 */d3.dgenies.switch_color_theme=function(){if(!d3.dgenies.all_disabled){let current_theme=d3.dgenies.color_idy_theme;let idx=d3.dgenies.color_idy_themes.indexOf(current_theme);if(idx<d3.dgenies.color_idy_themes.length-1){idx++}else{idx=0}d3.dgenies.change_color_theme(d3.dgenies.color_idy_themes[idx])}};d3.dgenies.set_track_color=function(val){if(val!="")return val;else return d3.dgenies.track_default_color};/**
 * Set track color template for right track
 */d3.dgenies.set_right_track_color_tmpl=function(){if(d3.dgenies.y_track&&d3.dgenies.y_track.type=="wig"){let color_theme=d3.dgenies.color_idy[d3.dgenies.color_idy_theme]["track_heatmap"];d3.dgenies.right_track_color=d3.scaleSequential().interpolator(color_theme).domain(d3.dgenies.y_track.range)}else{d3.dgenies.right_track_color=d3.dgenies.color_idy[d3.dgenies.color_idy_theme]["track_color"]}};/**
 * Set track color template for top track
 */d3.dgenies.set_top_track_color_tmpl=function(){if(d3.dgenies.x_track&&d3.dgenies.x_track.type=="wig"){let color_theme=d3.dgenies.color_idy[d3.dgenies.color_idy_theme]["track_heatmap"];d3.dgenies.top_track_color=d3.scaleSequential().interpolator(color_theme).domain(d3.dgenies.x_track.range)}else{d3.dgenies.top_track_color=d3.dgenies.color_idy[d3.dgenies.color_idy_theme]["track_color"]}};/**
 * Change color theme to the given one
 *
 * @param {string} theme theme name
 */d3.dgenies.change_color_theme=function(theme){if(d3.dgenies.color_idy_themes.indexOf(theme)===-1){throw"Theme not valid!"}for(let idy=0;idy<4;idy++){d3.dgenies.color_idy_theme=theme;d3.dgenies.container.selectAll("path.idy_"+idy.toString()).attr("stroke",d3.dgenies.color_idy[d3.dgenies.color_idy_theme]["palette"][idy])}// Update top track
d3.dgenies.set_top_track_color_tmpl();d3.dgenies.top_track_container.selectAll("rect.track").each(function(d,i){self=d3.select(this);self.attr("fill",d3.dgenies.top_track_color(d3.dgenies.set_track_color(self.attr("fill_origin"))))});// Update right track
d3.dgenies.set_right_track_color_tmpl();d3.dgenies.right_track_container.selectAll("rect.track").each(function(d,i){self=d3.select(this);self.attr("fill",d3.dgenies.right_track_color(d3.dgenies.set_track_color(self.attr("fill_origin"))))});d3.dgenies.draw_legend()};/**
...
$("path.content-lines").remove();for(let i=0;i<4;i++){d3.dgenies.__draw_idy_lines(i.toString(),lines,x_len,y_len)}d3.dgenies.events.filter_size(d3.dgenies.min_size)};d3.dgenies.create_pickr=function(el,default_color="#FFFFFF00"){return new Pickr({el:el,default:default_color,theme:"nano",lockOpacity:false,position:"bottom-start",swatches:["rgba(244, 67, 54, 0.2)","rgba(233, 30, 99, 0.2)","rgba(156, 39, 176, 0.2)","rgba(103, 58, 183, 0.2)","rgba(63, 81, 181, 0.2)","rgba(33, 150, 243, 0.2)","rgba(3, 169, 244, 0.2)","rgba(0, 188, 212, 0.2)","rgba(0, 150, 136, 0.2)","rgba(76, 175, 80, 0.2)","rgba(139, 195, 74, 0.2)","rgba(205, 220, 57, 0.2)","rgba(255, 235, 59, 0.2)","rgba(255, 193, 7, 0.2)","rgba(128, 128, 128, 0.2)","rgba(255, 255, 255, 0)"],components:{palette:false,preview:false,opacity:false,hue:false,interaction:{hex:false,rgba:false,hsva:false,input:false,clear:false,save:false}}})};d3.dgenies.add_restore_all=function(){let draw=$("#draw");draw.append($("<div>").attr("class","restore-all").html(`
                ...
                `));d3.dgenies.x_pickr=d3.dgenies.create_pickr("#xpickr",d3.dgenies.x_background_color).on("change",(color,source,instance)=>{instance.applyColor()}).on("save",(color,instance)=>{d3.dgenies.x_background_color=color!==null?color.toRGBA().toString():"rgba(255,255,255,0)";d3.dgenies.draw_background(d3.dgenies.x_order,d3.dgenies.y_order)});$("#rowColor button").addClass("btn btn-sm").attr("id","xpickr").after(`<label for="xpickr">Column</label>`);d3.dgenies.y_pickr=d3.dgenies.create_pickr("#ypickr",d3.dgenies.y_background_color).on("change",(color,source,instance)=>{console.log("Event: \"change\"",color,source,instance);instance.applyColor()}).on("save",(color,instance)=>{d3.dgenies.y_background_color=color!==null?color.toRGBA().toString():"rgba(255,255,255,0)";d3.dgenies.draw_background(d3.dgenies.x_order,d3.dgenies.y_order)});$("#columnColor button").addClass("btn btn-sm").attr("id","ypickr").after(`<label for="ypickr">Row</label>`);if(d3.dgenies.x_track!=null){$("#targetTrackSwitch").on("change",d3.dgenies.toggle_top_track)}else{$("#targetTrackSwitch").prop("checked",false).prop("disabled",true);//setTimeout(() => { $("#targetTrackSwitch").prop("disabled", true) }, 1000);
}if(d3.dgenies.y_track!=null){$("#queryTrackSwitch").on("change",d3.dgenies.toggle_right_track)}else{$("#queryTrackSwitch").prop("checked",false).prop("disabled",true)}};/**
 * Draw dot plot
 *
 ...
 */d3.dgenies.draw=function(x_contigs,x_order,y_contigs,y_order){let width=850;let height=850;$("div#draw").width(width).height(height);let draw=$("#draw");draw.empty();d3.dgenies.add_restore_all();let draw_in=draw.append($("<div>").attr("id","draw-in"));d3.dgenies.svgsupercontainer=d3.select("#draw-in").append("svg:svg").attr("width","100%").attr("height","100%").attr("viewBox","0 0 100 100").attr("preserveAspectRatio","none");let drawcontainer=d3.dgenies.svgsupercontainer.append("svg:g").attr("class","drawcontainer");drawcontainer.append("rect").attr("x",0).attr("y",0).attr("width","100%").attr("height","100%").attr("stroke","none").attr("fill","white");d3.dgenies.svgcontainer=drawcontainer.append("svg:svg").attr("class","svgcontainer").attr("width",d3.dgenies.container_dotplot_width-d3.dgenies.container_axis_thickness).attr("height",d3.dgenies.container_dotplot_height-d3.dgenies.container_axis_thickness).attr("x",d3.dgenies.container_axis_thickness).attr("y",d3.dgenies.container_axis_thickness+d3.dgenies.container_track_thickness).attr("viewBox","0 0 "+d3.dgenies.scale+" "+d3.dgenies.scale).attr("preserveAspectRatio","none");d3.dgenies.container=d3.dgenies.svgcontainer.append("svg:g").attr("class","container");d3.dgenies.container.append("rect").attr("x",0).attr("y",0).attr("width","100%").attr("height","100%").attr("stroke","none").attr("fill","white");if(d3.dgenies.x_track!==null){let svg_top=d3.dgenies.svgsupercontainer.append("svg:svg").attr("class","top-track track-container").attr("width",d3.dgenies.container_dotplot_width-d3.dgenies.container_axis_thickness).attr("height",d3.dgenies.container_track_thickness).attr("x",d3.dgenies.container_axis_thickness).attr("y",d3.dgenies.container_axis_thickness).attr("viewBox","0 0 "+d3.dgenies.scale+" 1000").attr("preserveAspectRatio","none");d3.dgenies.top_track_container=svg_top.append("svg:g").attr("class","top-track-container");d3.dgenies.top_track_container.append("rect").attr("x",0).attr("y",0).attr("width","100%").attr("height","100%").attr("stroke","none").attr("fill","white")}if(d3.dgenies.y_track!==null){let svg_right=d3.dgenies.svgsupercontainer.append("svg:svg").attr("class","right-track track-container").attr("width",d3.dgenies.container_track_thickness).attr("height",d3.dgenies.container_dotplot_height-d3.dgenies.container_axis_thickness).attr("x",d3.dgenies.container_dotplot_width).attr("y",d3.dgenies.container_axis_thickness+d3.dgenies.container_track_thickness).attr("viewBox","0 0 1000 "+d3.dgenies.scale).attr("preserveAspectRatio","none");d3.dgenies.right_track_container=svg_right.append("svg:g").attr("class","right-track-container");d3.dgenies.right_track_container.append("rect").attr("x",0).attr("y",0).attr("width","100%").attr("height","100%").attr("stroke","none").attr("fill","white")}//X axis:
d3.dgenies.x_zones={};let sum=0;for(let i=0;i<x_order.length-1;i++){let x_id=x_order[i];let x_contig_len=x_contigs[x_id]/d3.dgenies.x_len*d3.dgenies.scale;d3.dgenies.x_zones[x_id]=[sum,sum+x_contig_len];sum+=x_contig_len}d3.dgenies.x_zones[x_order[x_order.length-1]]=[sum,d3.dgenies.scale];//Y axis:
d3.dgenies.y_zones={};sum=0;for(let i=0;i<y_order.length-1;i++){let y_id=y_order[i];let y_contig_len=y_contigs[y_id]/d3.dgenies.y_len*d3.dgenies.scale;d3.dgenies.y_zones[y_id]=[sum,sum+y_contig_len];sum+=y_contig_len}d3.dgenies.y_zones[y_order[y_order.length-1]]=[sum,d3.dgenies.scale];d3.dgenies.background=d3.dgenies.container.append("svg:g").attr("id","dotplot-background");d3.dgenies.draw_background(x_order,y_order);d3.dgenies.draw_x_lines(x_order);d3.dgenies.draw_y_lines(y_order);if(!d3.dgenies.break_lines_show){d3.selectAll("line.break-lines").style("visibility","hidden")}d3.dgenies.draw_axis_bckgd();d3.dgenies.draw_left_axis(d3.dgenies.y_len);d3.dgenies.draw_bottom_axis(d3.dgenies.x_len);d3.dgenies.draw_top_axis(d3.dgenies.x_zones);d3.dgenies.draw_right_axis(d3.dgenies.y_zones);window.setTimeout(()=>{//Data:
d3.dgenies.draw_lines();d3.dgenies.draw_top_track();d3.dgenies.draw_right_track();if(d3.dgenies.x_track==null&&d3.dgenies.y_track==null){d3.dgenies.hide_track_container()}$("#restore-all").click(function(){if(d3.dgenies.zoom.reset_scale(false,null,false)){$(this).prop("disabled",true)}});$(document).on("keyup",function(e){if(e.keyCode===27){if(d3.dgenies.zoom.reset_scale(false,null,false)){$("#restore-all").prop("disabled",true)}}});d3.dgenies.draw_legend();dgenies.hide_loading()},0);draw.append($("<div>").attr("id","help-zoom").append("Press CTRL to zoom").append($("<img>").attr("src",d3.dgenies.help_zoom).attr("alt","")).hide().on("click",function(){$(this).hide()}));draw.append($("<div>").attr("id","help-trans").append("Press CTRL to translate").append($("<img>").attr("src",d3.dgenies.help_trans).attr("alt","")).hide().on("click",function(){$(this).hide()}).on("mouseup",function(){d3.dgenies.translate_start=null;$("#help-trans").fadeOut("slow")}));d3.dgenies.zoom.init();d3.dgenies.events.init_context_menu()};if(!d3||!d3.dgenies){throw"d3.dgenies wasn't included!"}d3.dgenies.events={};d3.dgenies.events.context_menu={actions:[{name:"Export SVG",onClick:dgenies.result.export.export_svg},{name:"Export PNG",onClick:dgenies.result.export.export_png},{name:"Reverse query",isShown:function(){return d3.dgenies.name_x!==d3.dgenies.name_y},onClick:dgenies.result.controls.launch_reverse_contig}]};/**
 * Initialise events
 ...
 */d3.dgenies.events.stroke_width=function(width){let stroke_width=d3.dgenies.scale/600;if(width==="1"){stroke_width=d3.dgenies.scale/400}else if(width==="2"){stroke_width=d3.dgenies.scale/200}else if(width==="3"){stroke_width=d3.dgenies.scale/100}d3.dgenies.content_lines_width=stroke_width;d3.selectAll("path.content-lines").attr("stroke-width",stroke_width/d3.dgenies.zoom_scale_lines)};if(!d3||!d3.dgenies){throw"d3.dgenies wasn't included!"}d3.dgenies.mousetip={};d3.dgenies.mousetip.dotplot={};d3.dgenies.mousetip.track_x={};d3.dgenies.mousetip.track_y={};/**
 ...
 * @param {int} x
 * @param {int} y
 */$.fn.mousetip_dotplot=function(my_tip,relative_to=null,x=20,y=20){let $this=$(this);let tip=relative_to===null?$(my_tip,this):$(my_tip,relative_to);let hidden=true;$this.hover(function(e){if(!e.ctrlKey){tip.show();hidden=false}},function(){hidden=true;tip.hide().removeAttr("style")}).mousemove(function(e){tip.hide();window.setTimeout(()=>{let rect=relative_to===null?this.getBoundingClientRect():$(relative_to)[0].getBoundingClientRect();let posX=rect.left+window.scrollX,posY=rect.top+window.scrollY,m_x=e.pageX-rect.left-window.scrollX,m_y=e.pageY-rect.top-window.scrollY;let mouseX=m_x+x;let mouseY=m_y+y;let rect_g=$("g.container")[0].getBoundingClientRect();let posX_g=rect_g.left+window.scrollX,posY_g=rect_g.top+window.scrollY,width_c=rect_g.width,height_c=rect_g.height;let x_g=(e.pageX-posX_g)/width_c*d3.dgenies.scale,y_g=d3.dgenies.scale-(e.pageY-posY_g)/height_c*d3.dgenies.scale;let match=d3.dgenies.mousetip.dotplot.get_match(e);let x_zone="unknown";for(let zone in d3.dgenies.x_zones){if(d3.dgenies.x_zones[zone][0]<x_g&&x_g<=d3.dgenies.x_zones[zone][1]){x_zone=d3.dgenies.mousetip.get_label(zone);break}}let y_zone="unknown";for(let zone in d3.dgenies.y_zones){if(d3.dgenies.y_zones[zone][0]<y_g&&y_g<=d3.dgenies.y_zones[zone][1]){y_zone=d3.dgenies.mousetip.get_label(zone);break}}let html="";if(match!==null){html=`<strong>Query:</strong> ${match.y_zone}<br/>(${match.y_match[0]} - ${match.y_match[1]})<br/>
                    ...
                    </table>`}tip.html(html);let css={top:mouseY,left:mouseX};if(match===null){css.color="#000";css.background="#ededed"}else{let idy_class="";if(match.idy>=0.75){idy_class="3"}else if(match.idy>=0.5){idy_class="2"}else if(match.idy>=0.25){idy_class="1"}else if(match.idy>=0){idy_class="0"}else{idy_class="-1"}css.background=d3.dgenies.color_idy[d3.dgenies.color_idy_theme]["palette"][idy_class];css.color=d3.dgenies.mousetip.getColorByBgColor(css.background)}if(!hidden&&(!e.ctrlKey&&!d3.dgenies.zone_selected||match!==null)){tip.show().css(css)}},0)})};$.fn.mousetip_track_x=function(my_tip,relative_to=null,x=20,y=20){let $this=$(this);console.log($this);let tip=relative_to===null?$(my_tip,this):$(my_tip,relative_to);let hidden=true;$this.hover(function(e){if(!e.ctrlKey){tip.show();hidden=false}},function(){hidden=true;tip.hide().removeAttr("style")}).mousemove(function(e){tip.hide();window.setTimeout(()=>{let rect=relative_to===null?this.getBoundingClientRect():$(relative_to)[0].getBoundingClientRect();let posX=rect.left+window.scrollX,posY=rect.top+window.scrollY,m_x=e.pageX-rect.left-window.scrollX,m_y=e.pageY-rect.top-window.scrollY;let mouseX=m_x+x;let mouseY=m_y+y;let match=d3.dgenies.mousetip.track_x.get_match(e);let html="";if(match!==null){html=`<strong>Target:</strong> ${match.contig}<br/>(${match.feature[0]} - ${match.feature[0]+match.feature[1]})<br/>
                        <strong>Value:</strong> ${match.feature[2]}<br/>
                        <strong>Comment:</strong> ${match.feature[3]}`}tip.html(html);let css={top:mouseY,left:mouseX};console.log(tip);if(!hidden&&!e.ctrlKey&&!d3.dgenies.zone_selected&&match!==null){tip.show().css(css)}},0)})};$.fn.mousetip_track_y=function(my_tip,relative_to=null,x=20,y=20){let $this=$(this);console.log($this);let tip=relative_to===null?$(my_tip,this):$(my_tip,relative_to);let hidden=true;$this.hover(function(e){if(!e.ctrlKey){tip.show();hidden=false}},function(){hidden=true;tip.hide().removeAttr("style")}).mousemove(function(e){tip.hide();window.setTimeout(()=>{let rect=relative_to===null?this.getBoundingClientRect():$(relative_to)[0].getBoundingClientRect();let posX=rect.left+window.scrollX,posY=rect.top+window.scrollY,m_x=e.pageX-rect.left-window.scrollX,m_y=e.pageY-rect.top-window.scrollY;let mouseX=m_x+x;let mouseY=m_y+y;let match=d3.dgenies.mousetip.track_y.get_match(e);let html="";if(match!==null){html=`<strong>Query:</strong> ${match.contig}<br/>(${match.feature[0]} - ${match.feature[0]+match.feature[1]})<br/>
                        <strong>Value:</strong> ${match.feature[2]}<br/>
                        <strong>Comment:</strong> ${match.feature[3]}`}tip.html(html);let css={top:mouseY,left:mouseX};if(!hidden&&!e.ctrlKey&&!d3.dgenies.zone_selected&&match!==null){tip.show().css(css)}},0)})};/**
 * Initialise tooltips
 */d3.dgenies.mousetip.init=function(){d3.dgenies.mousetip.dotplot.init();d3.dgenies.mousetip.track_x.init();d3.dgenies.mousetip.track_y.init()};/**
 * Hide tooltips
 ...
 */d3.dgenies.mousetip.dotplot.init=function(){$("#draw").append($("<span>").attr("class","tip"));$("g.container").mousetip_dotplot(".tip","#draw")};/**
 * Get match override by mouse cursor on dotplot
 *
 * @param e mouse event
 * @returns {{x_zone: string, y_zone: string, x_match: float[], y_match: float[], idy: float}}
 */d3.dgenies.mousetip.dotplot.get_match=function(e){let rect=$("g.container")[0].getBoundingClientRect();let posX=rect.left+window.scrollX,posY=rect.top+window.scrollY,width_c=rect.width,height_c=rect.height;let c_x=(e.pageX-posX)/width_c*d3.dgenies.scale,c_y=d3.dgenies.scale-(e.pageY-posY)/height_c*d3.dgenies.scale;c_x=c_x/d3.dgenies.scale*d3.dgenies.x_len;c_y=c_y/d3.dgenies.scale*d3.dgenies.y_len;let error_x=d3.dgenies.content_lines_width/d3.dgenies.scale*d3.dgenies.x_len;let error_y=d3.dgenies.content_lines_width/d3.dgenies.scale*d3.dgenies.y_len;// let error_x = 0,
//     error_y = 0;
 ...
 * Initialise tooltip for dotplot
 */d3.dgenies.mousetip.track_x.init=function(){$("g.top-track-container").mousetip_track_x(".tip","#draw")};/**
 * Get match override by mouse cursor on track x
 *
 * @param e mouse event
 * @returns {{x_zone: string, y_zone: string, x_match: float[], y_match: float[], idy: float}}
 */d3.dgenies.mousetip.track_x.get_match=function(e){// Get DOM object coordinates the mouse cursor is over.
let rect=$("g.top-track-container")[0].getBoundingClientRect();let posX=rect.left+window.scrollX,posY=rect.top+window.scrollY,width_c=rect.width,height_c=rect.height;console.log(posX,posY,width_c,height_c);// Transform into coordinates in data
let c_x=(e.pageX-posX)/width_c*d3.dgenies.scale;c_x=c_x/d3.dgenies.scale*d3.dgenies.x_len;console.log(c_x);let error_x=d3.dgenies.content_lines_width/d3.dgenies.scale*d3.dgenies.x_len;// let error_x = 0,
let match=null;let found=false;let x_track=d3.dgenies.x_track.data;for(let x_id in x_track){for(let feat of x_track[x_id]){let f_start=feat[0];let f_len=feat[1];if(f_start<=c_x&&c_x<=f_start+f_len){match={contig:x_id,feature:feat};found=true;break}}if(found){break}}return match};/**
 * Initialise tooltip for dotplot
 */d3.dgenies.mousetip.track_y.init=function(){$("g.right-track-container").mousetip_track_y(".tip","#draw")};/**
 * Get match override by mouse cursor on track y
 *
 * @param e mouse event
 * @returns {{x_zone: string, y_zone: string, x_match: float[], y_match: float[], idy: float}}
 */d3.dgenies.mousetip.track_y.get_match=function(e){// Get DOM object coordinates the mouse cursor is over.
let rect=$("g.right-track-container")[0].getBoundingClientRect();let posX=rect.left+window.scrollX,posY=rect.top+window.scrollY,width_c=rect.width,height_c=rect.height;// Transform into coordinates in data
let c_y=d3.dgenies.scale-(e.pageY-posY)/height_c*d3.dgenies.scale;c_y=c_y/d3.dgenies.scale*d3.dgenies.y_len;let error_y=d3.dgenies.content_lines_width/d3.dgenies.scale*d3.dgenies.y_len;// let error_x = 0,
let match=null;let found=false;let x_track=d3.dgenies.y_track.data;for(let x_id in x_track){for(let feat of x_track[x_id]){let f_start=feat[0];let f_len=feat[1];if(f_start<=c_y&&c_y<=f_start+f_len){match={contig:x_id,feature:feat};found=true;break}}if(found){break}}return match};if(!d3||!d3.dgenies){throw"d3.dgenies wasn't included!"}d3.dgenies.zoom={};d3.dgenies.zoom.help_timeout=null;/**
...
d3.dgenies.draw_top_axis();d3.dgenies.draw_right_axis();d3.dgenies.zoom_bottom_axis();d3.dgenies.zoom_left_axis();d3.dgenies.zoom_top_track(new_scale);d3.dgenies.zoom_right_track(new_scale);if(translate_x<=0.00001&&translate_x>=-0.00001&&translate_y<=0.00001&&translate_y>=-0.00001&&new_scale===1){$("#restore-all").prop("disabled",true)}else{$("#restore-all").prop("disabled",false)}}}else if(d3.dgenies.zoom_enabled){let help_zoom=$("#help-zoom");help_zoom.fadeIn("slow");if(d3.dgenies.zoom.help_timeout!==null){window.clearTimeout(d3.dgenies.zoom.help_timeout)}d3.dgenies.zoom.help_timeout=window.setTimeout(()=>{help_zoom.fadeOut("slow")},700)}};/**
...
d3.dgenies.draw_top_axis();d3.dgenies.draw_right_axis();//Update tracks:
d3.dgenies.zoom_right_track();d3.dgenies.zoom_top_track();if(!temp)d3.dgenies.zone_selected=false;dgenies.hide_loading();//Re-enable zoom:
