...
d3.dgenies.scale=1000;d3.dgenies.axis_length=500;d3.dgenies.content_lines_width=d3.dgenies.scale/400;d3.dgenies.break_lines_width=d3.dgenies.scale/1500;d3.dgenies.color_idy_theme="default";d3.dgenies.color_idy_themes=["default","colorblind","greyscale","r_default","r_colorblind","allblack"];d3.dgenies.color_background=["#ffffff","#f9f9f9"];d3.dgenies.color_idy={"default":{"type":"normal","palette":{"3":"#094b09","2":"#2ebd40","1":"#d5670b","0":"#ffd84b","-1":"#fff"}},"colorblind":{"type":"colorblind","palette":{"3":"#000","2":"#006DDB","1":"#DB6E00","0":"#FFB677","-1":"#fff"}},"greyscale":{"type":"greyscale","palette":{"3":"#000","2":"#626262","1":"#9c9c9c","0":"#DDDCDC","-1":"#fff"}},"r_default":{"type":"normal","palette":{"3":"#7fff65","2":"#238d31","1":"#78410d","0":"#3b080a","-1":"#fff"}},"r_colorblind":{"type":"colorblind","palette":{"3":"#8c8c8c","2":"#006DDB","1":"#783c00","0":"#312515","-1":"#fff"}},"allblack":{"type":"bw","palette":{"3":"#000","2":"#000","1":"#000","0":"#000","-1":"#fff"}}};d3.dgenies.limit_idy=null;d3.dgenies.min_idy_draw=0;d3.dgenies.min_size=0;d3.dgenies.linecap="round";d3.dgenies.background_axis="#f4f4f4";d3.dgenies.break_lines_color="#7c7c7c";d3.dgenies.break_lines_dash="3, 3";d3.dgenies.break_lines_show=true;d3.dgenies.zoom_scale_lines=1;// Zoom scale used for lines width
...
//s = +(s * 100).toFixed(1);
l=+(l*100).toFixed(1);return"hsl("+0+","+0+"%,"+l+"%)"};/**
 ...
 */d3.dgenies.apply_theme_on_track=function(hex){let theme_type=d3.dgenies.color_idy[d3.dgenies.color_idy_theme]["type"];if(theme_type=="bw"){return"#000000"}if(theme_type=="greyscale"){return d3.dgenies.hex_to_greyscale(hex)}return hex};/**
 * Draw top track
 */d3.dgenies.draw_top_track=function(){let svg_top=d3.dgenies.svgsupercontainer.append("svg:svg").attr("class","top-track").attr("width",d3.dgenies.container_dotplot_width-d3.dgenies.container_axis_thickness).attr("height",d3.dgenies.container_track_thickness).attr("x",d3.dgenies.container_axis_thickness).attr("y",d3.dgenies.container_axis_thickness).attr("viewBox","0 0 "+d3.dgenies.scale+" 1000").attr("preserveAspectRatio","none");svg_top.append("rect").attr("x",0).attr("y",0).attr("width","100%").attr("height","100%").attr("fill","#ffffff").attr("stroke","black");d3.dgenies.top_track_container=svg_top.append("svg:g").attr("class","top-track-container");if(d3.dgenies.x_track!==null){const track_type=d3.dgenies.x_track.type;let x_track=d3.dgenies.x_track.data;for(let x_id in x_track){for(let feat of x_track[x_id]){let f_start=feat[0];let f_len=feat[1];let f_rep=feat[2];let f_val=feat[3];let f_comment=feat[4];let start=f_start/d3.dgenies.x_len*d3.dgenies.scale;length=f_len/d3.dgenies.x_len*d3.dgenies.scale;let block=d3.dgenies.top_track_container.append("rect").attr("x",start).attr("y",0).attr("width",length).attr("height","100%").attr("fill","#0000ff").attr("stroke","none").attr("class","track");if(f_val!=""){block.attr("fill_origin",f_val);block.attr("fill",f_val)}}}}};/**
 * Draw right track
 */d3.dgenies.draw_right_track=function(){let svg_right=d3.dgenies.svgsupercontainer.append("svg:svg").attr("class","right-track").attr("width",d3.dgenies.container_track_thickness).attr("height",d3.dgenies.container_dotplot_height-d3.dgenies.container_axis_thickness).attr("x",d3.dgenies.container_dotplot_width).attr("y",d3.dgenies.container_axis_thickness+d3.dgenies.container_track_thickness).attr("viewBox","0 0 1000 "+d3.dgenies.scale).attr("preserveAspectRatio","none");svg_right.append("rect").attr("x",0).attr("y",0).attr("width","100%").attr("height","100%").attr("fill","#ffffff").attr("stroke","black");d3.dgenies.right_track_container=svg_right.append("svg:g").attr("class","right-track-container");if(d3.dgenies.y_track!==null){const track_type=d3.dgenies.y_track.type;let y_track=d3.dgenies.y_track.data;for(let y_id in y_track){for(let feat of y_track[y_id]){let f_start=feat[0];let f_len=feat[1];let f_rep=feat[2];let f_val=feat[3];let f_comment=feat[4];let start=(d3.dgenies.y_len-f_start-f_len)/d3.dgenies.y_len*d3.dgenies.scale;let length=f_len/d3.dgenies.y_len*d3.dgenies.scale;let block=d3.dgenies.right_track_container.append("rect").attr("x",0).attr("y",start).attr("width","100%").attr("height",length).attr("fill","#0000ff").attr("stroke","none").attr("class","track");if(f_val!=""){block.attr("fill_origin",f_val);block.attr("fill",f_val)}}}}};/**
 * Zoom top track
 * 
 ...
 */d3.dgenies.change_color_theme=function(theme){if(d3.dgenies.color_idy_themes.indexOf(theme)===-1){throw"Theme not valid!"}for(let idy=0;idy<4;idy++){d3.dgenies.color_idy_theme=theme;d3.dgenies.container.selectAll("path.idy_"+idy.toString()).attr("stroke",d3.dgenies.color_idy[d3.dgenies.color_idy_theme]["palette"][idy])}d3.dgenies.top_track_container.selectAll("rect.track").each(function(d,i){self=d3.select(this);self.attr("fill",d3.dgenies.apply_theme_on_track(self.attr("fill_origin")))});d3.dgenies.right_track_container.selectAll("rect.track").each(function(d,i){self=d3.select(this);self.attr("fill",d3.dgenies.apply_theme_on_track(self.attr("fill_origin")))});d3.dgenies.draw_legend()};/**
