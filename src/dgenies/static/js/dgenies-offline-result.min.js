 ...
 */d3.dgenies.launch=function(res,update=false,noise_change=false){dgenies.fill_select_zones(res["x_order"],res["y_order"]);if(res["sorted"]){$("input#sort-contigs").val("Undo sort");$("#export").find("select option[value=4]").show();$("#export").find("select option[value=8]").show()}else{$("input#sort-contigs").val("Sort contigs");$("#export").find("select option[value=4]").hide();$("#export").find("select option[value=8]").hide()}d3.dgenies.name_x=res["name_x"];d3.dgenies.name_y=res["name_y"];if(d3.dgenies.name_x===d3.dgenies.name_y){$("input#sort-contigs").hide()}else{$("input#sort-contigs").show()}d3.dgenies.lines=res["lines"];d3.dgenies.x_len=res["x_len"];d3.dgenies.y_len=res["y_len"];d3.dgenies.min_idy=res["min_idy"];d3.dgenies.max_idy=res["max_idy"];d3.dgenies.limit_idy=res["limit_idy"];if(!noise_change){dgenies.noise=true}$("#hide-noise").val(dgenies.noise?"Hide noise":"Show noise");d3.dgenies.draw(res["x_contigs"],res["x_order"],res["y_contigs"],res["y_order"]);if(!update){$("div#draw").resizable({aspectRatio:true});d3.dgenies.events.init();dgenies.result.controls.init()}if(res["sampled"]){let max_nb_lines=dgenies.numberWithCommas(res["max_nb_lines"].toString());dgenies.notify(`<div style="text-align: center"><b>There are too much matches.\nOnly the ${max_nb_lines} best matches are displayed</b></div>`)}d3.dgenies.mousetip.init()};/**
