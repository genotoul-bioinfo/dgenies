 ...
 */dgenies.init=function(all_jobs,mode){dgenies.mode=mode;let cookies=$.cookie("results");if(mode==="webserver"){cookies=cookies!==undefined&&cookies.length>0?cookies.split("|"):[]}else{cookies=all_jobs;dgenies.save_cookies(cookies)}dgenies.update_results(cookies)};/**
 * Save cookie on the browser
 * @param {array} cookies list of jobs
 */dgenies.save_cookies=function(cookies){$.cookie("results",cookies.join("|"),{path:"/"})};/**
 * Update list of jobs
 * @param {array} results: new list of jobs
 */dgenies.update_results=function(results){let job_list_item=$("ul.nav li.result ul");job_list_item.html("");if(results.length>0){for(let i=0;i<results.length;i++){let result=results[i];job_list_item.append($("<li>").append($("<a>").attr("href",`/result/${result}`).text(result)))}}else{job_list_item.append($("<li>").append($("<a>").attr("href","/run").text("Please run a job!")))}};/**
 ...
 * Hide loading popup
 */dgenies.hide_loading=function(){$("input,form#export select").prop("disabled",false);d3.boxplot.all_disabled=false;$(dgenies.loading).hide();dgenies.reset_loading_message()};/**
 ...
 */dgenies.result.init=function(id_res){dgenies.result.id_res=id_res;dgenies.result.add_to_list();d3.boxplot.init()};/**
 ...
 */dgenies.result.export.save_file=function(blob,format){dgenies.hide_loading();saveAs(blob,`map_${d3.boxplot.name_y}_to_${d3.boxplot.name_x}.${format}`)};/**
 ...
 */dgenies.result.export.export_svg=function(){dgenies.show_loading("Building picture...",180);window.setTimeout(()=>{let svg="<?xml version=\"1.0\" encoding=\"utf-8\"?>\n<!DOCTYPE svg PUBLIC \"-//W3C//DTD SVG 1.1//EN\" "+"\"http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd\">";svg+=dgenies.result.export.get_svg("1000px");let blob=new Blob([svg],{type:"image/svg+xml"});//d3.boxplot.zoom.restore_scale(transform);
 ...
 */dgenies.result.export.dl_fasta=function(gzip=false){let export_div=$("div#export-pict");export_div.html("");export_div.append($("<a>").attr("href",`/fasta-query/${dgenies.result.id_res}`).attr("download",d3.boxplot.name_y+(gzip?".fasta.gz":".fasta")).attr("id","my-download").text("download"));dgenies.hide_loading();document.getElementById("my-download").click()};/**
 * Export fasta file
 ...
 */dgenies.result.export.export_backup_file=function(){dgenies.show_loading("Building file...",180);window.setTimeout(()=>{let export_div=$("div#export-pict");export_div.html("");export_div.append($("<a>").attr("href",`/backup/${dgenies.result.id_res}`).attr("download",d3.boxplot.name_y+"_"+d3.boxplot.name_x+"_"+dgenies.result.id_res+".tar.gz").attr("id","my-download").text("download"));dgenies.hide_loading();document.getElementById("my-download").click()})};/**
 ...
 * @param {string} format file format
 */dgenies.result.summary.save_file=function(blob,format){saveAs(blob,`summary_${d3.boxplot.name_y}_to_${d3.boxplot.name_x}.${format}`)};/**
 ...
 */d3.boxplot.init=function(id_res=null,from_file=false){if(id_res===null){id_res=dgenies.result.id_res}$("#form-parameters")[0].reset();$("form#select-zone")[0].reset();if(!from_file){dgenies.post("/get_graph",{"id":id_res},function(data){if(data["success"]){d3.boxplot.launch(data)}else{$("#supdraw").html($("<p>").html("message"in data?data["message"]:"This job does not exist!").css("margin-top","15px"));dgenies.result.remove_job_from_cookie(dgenies.result.id_res)}})}else{dgenies.get(id_res,{},function(data){d3.boxplot.launch(data)})}};/**
 ...
 */d3.boxplot.select_zone=function(x=null,y=null,x_zone=null,y_zone=null,force=false){d3.boxplot.mousetip.hide();dgenies.show_loading();window.setTimeout(()=>{if(!d3.boxplot.zone_selected||force){if(x_zone===null){//Search zone for X axis:
...
let pseudo_x_zones={};pseudo_x_zones[x_zone]=[0,d3.boxplot.x_len];d3.boxplot.draw_top_axis(pseudo_x_zones);let pseudo_y_zones={};pseudo_y_zones[y_zone]=[0,d3.boxplot.y_len];d3.boxplot.draw_right_axis(pseudo_y_zones);d3.boxplot.zoom_enabled=false}$("#restore-all").show();dgenies.hide_loading()},0)};/**
 ...
 */d3.boxplot.get_human_readable_size=function(nbases,precision=1,space=" "){let lab="";let prec=parseInt("1"+"0".repeat(precision));if(nbases>1000000){lab=(Math.round(nbases/(1000000/prec))/prec).toString()+space+"M"}else if(nbases>1000){lab=(Math.round(nbases/(1000/prec))/prec).toString()+space+"K"}else{lab=Math.round(nbases).toString()}return lab};/**
 * Draw left axis
 *
 * @param {int} y_max max value of y on the Y axis
 * @param {int} y_min min value of y on the Y axis
 */d3.boxplot.draw_left_axis=function(y_max,y_min=0){let axis_length=500;$("svg.left-axis").remove();//Remove previous axis (if any)
let svg_left=d3.boxplot.svgsupercontainer.append("svg:svg").attr("class","axis left-axis").attr("width",5).attr("height",90).attr("x",0).attr("y",5).attr("viewBox","0 0 20 "+axis_length).attr("preserveAspectRatio","none");let container_left=svg_left.append("g").attr("transform","translate(0,"+axis_length+")rotate(-90)");let y_size=y_max-y_min;for(let i=1;i<10;i++){let y=axis_length/10*i;let y_t=y_min+y_size/10*i;if(y_t>=0&&y_t<=d3.boxplot.y_len){let y_lab=d3.boxplot.get_human_readable_size(y_t);container_left.append("line").attr("x1",y).attr("y1",15).attr("x2",y).attr("y2",20).attr("stroke-width",d3.boxplot.tick_width).attr("stroke","black");container_left.append("text").attr("x",y).attr("y",12).attr("text-anchor","middle").attr("font-family","sans-serif").attr("font-size","6.5pt").text(y_lab)}}};/**
 * Draw bottom axis
 *
 * @param {int} x_max max value of x on the X axis
 * @param {int} x_min min value of x on the X axis
 */d3.boxplot.draw_bottom_axis=function(x_max,x_min=0){let axis_length=500;$("svg.bottom-axis").remove();//Remove previous axis (if any)
let svg_bottom=d3.boxplot.svgsupercontainer.append("svg:svg").attr("class","axis bottom-axis").attr("width",90).attr("height",5).attr("x",5).attr("y",95).attr("viewBox","0 0 "+axis_length+" 20").attr("preserveAspectRatio","none");let x_size=x_max-x_min;for(let i=1;i<10;i++){let x=axis_length/10*i;let x_t=x_min+x_size/10*i;if(x_t>=0&&x_t<=d3.boxplot.x_len){let x_lab=d3.boxplot.get_human_readable_size(x_t);svg_bottom.append("line").attr("x1",x).attr("y1",0).attr("x2",x).attr("y2",5).attr("stroke-width",d3.boxplot.tick_width).attr("stroke","black");svg_bottom.append("text").attr("x",x).attr("y",15).attr("text-anchor","middle").attr("font-family","sans-serif").attr("font-size","6.5pt").text(x_lab)}}};/**
 * Zoom on left axis
 */d3.boxplot.zoom_left_axis=function(){let transform=d3.boxplot.container.attr("transform");if(transform===null){transform="translate(0,0)scale(1)"}let tr_regex=/translate\(([^,]+),([^)]+)\)/;let translate=parseFloat(transform.match(tr_regex)[2]);let sc_regex=/scale\(([^,)]+)(,([^)]+))?\)/;let scale=parseFloat(transform.match(sc_regex)[3]!==undefined?transform.match(sc_regex)[3]:transform.match(sc_regex)[1]);let max_y=d3.boxplot.y_len+translate/d3.boxplot.scale*d3.boxplot.y_len/scale;let min_y=max_y-d3.boxplot.y_len/scale;d3.boxplot.draw_left_axis(max_y,min_y)};/**
 * Zoom on bottom axis
 */d3.boxplot.zoom_bottom_axis=function(){let transform=d3.boxplot.container.attr("transform");if(transform===null){transform="translate(0,0)scale(1)"}let tr_regex=/translate\(([^,]+),([^)]+)\)/;let translate=parseFloat(transform.match(tr_regex)[1]);let sc_regex=/scale\(([^,)]+)(,([^)]+))?\)/;let scale=parseFloat(transform.match(sc_regex)[1]);let min_x=-translate/d3.boxplot.scale*d3.boxplot.x_len/scale;let max_x=(d3.boxplot.x_len-translate/d3.boxplot.scale*d3.boxplot.x_len)/scale;d3.boxplot.draw_bottom_axis(max_x,min_x)};/**
 * Draw top axis
 *
 * @param {object} x_zones: name of chromosomes of the target
 */d3.boxplot.draw_top_axis=function(x_zones=d3.boxplot.x_zones){$("svg.top-axis").remove();//Remove previous axis (if any)
...
svg_top.append("line").attr("x1",x_pos_1/d3.boxplot.scale*axis_length).attr("x2",x_pos_1/d3.boxplot.scale*axis_length).attr("y1",12).attr("y2",20).attr("stroke","black").attr("stroke-width",d3.boxplot.tick_width)}nb_zone++}};/**
 * Draw right axis
 *
 * @param {object} y_zones name of contigs of the query
 */d3.boxplot.draw_right_axis=function(y_zones=d3.boxplot.y_zones){$("svg.right-axis").remove();//Remove previous axis (if any)
...
container_right.append("line").attr("x1",y_pos_1/d3.boxplot.scale*axis_length).attr("x2",y_pos_1/d3.boxplot.scale*axis_length).attr("y1",12).attr("y2",20).attr("stroke","black").attr("stroke-width",d3.boxplot.tick_width).attr("class","whereis")}nb_zone--}};/**
 * Draw backgrounds of all axis
 */d3.boxplot.draw_axis_bckgd=function(){// Top:
 ...
 * @returns {number}
 * @private
 */d3.boxplot._sort_lines=function(l1,l2){return d3.boxplot._get_line_len(l2)-d3.boxplot._get_line_len(l1)};/**
 ...
 * @returns {number}
 * @private
 */d3.boxplot._sort_lines_by_idy=function(l1,l2){return l1[4]-l2[4]};/**
 ...
 * @private
 */d3.boxplot.__draw_idy_lines=function(idy,lines,x_len,y_len){let min_sizes=d3.boxplot.min_sizes;for(let i=0;i<min_sizes.length;i++){let min_size=min_sizes[i];let max_size=i+1<min_sizes.length?min_sizes[i+1]:null;if(lines[idy].length>0){d3.boxplot.container.append("path").attr("d",d3.boxplot.__lineFunction(lines[idy],min_size,max_size,x_len,y_len)).attr("class","content-lines s_"+min_size.toString().replace(".","_")+" idy_"+idy).attr("stroke-width",d3.boxplot.content_lines_width+"px").attr("stroke",d3.boxplot.color_idy[d3.boxplot.color_idy_theme][idy]).attr("stroke-linecap",d3.boxplot.linecap)}}};/**
 * Switch to next color theme
 */d3.boxplot.switch_color_theme=function(){if(!d3.boxplot.all_disabled){let current_theme=d3.boxplot.color_idy_theme;let idx=d3.boxplot.color_idy_themes.indexOf(current_theme);if(idx<d3.boxplot.color_idy_themes.length-1){idx++}else{idx=0}d3.boxplot.change_color_theme(d3.boxplot.color_idy_themes[idx])}};/**
 * Change color theme to the given one
 *
 * @param {string} theme theme name
 */d3.boxplot.change_color_theme=function(theme){if(d3.boxplot.color_idy_themes.indexOf(theme)===-1){throw"Theme not valid!"}for(let idy=0;idy<4;idy++){d3.boxplot.color_idy_theme=theme;d3.boxplot.container.selectAll("path.idy_"+idy.toString()).attr("stroke",d3.boxplot.color_idy[d3.boxplot.color_idy_theme][idy])}d3.boxplot.draw_legend()};/**
 ...
 */d3.boxplot.draw_lines=function(lines=d3.boxplot.lines,x_len=d3.boxplot.x_len,y_len=d3.boxplot.y_len){//Remove old lines (if any):
$("path.content-lines").remove();//lines = lines.sort(d3.boxplot._sort_lines);
for(let i=0;i<4;i++){d3.boxplot.__draw_idy_lines(i.toString(),lines,x_len,y_len)}d3.boxplot.events.filter_size(d3.boxplot.min_size)};/**
 * Draw dot plot
 *
 ...
 * @param {int} x
 * @param {int} y
 */$.fn.mousetip=function(my_tip,relative_to=null,x=20,y=20){let $this=$(this);let tip=relative_to===null?$(my_tip,this):$(my_tip,relative_to);let hidden=true;$this.hover(function(e){if(!e.ctrlKey){tip.show();hidden=false}},function(){hidden=true;tip.hide().removeAttr("style")}).mousemove(function(e){tip.hide();window.setTimeout(()=>{let rect=relative_to===null?this.getBoundingClientRect():$(relative_to)[0].getBoundingClientRect();let posX=rect.left+window.scrollX,posY=rect.top+window.scrollY,m_x=e.pageX-rect.left-window.scrollX,m_y=e.pageY-rect.top-window.scrollY;let mouseX=m_x+x;let mouseY=m_y+y;let rect_g=$("g.container")[0].getBoundingClientRect();let posX_g=rect_g.left+window.scrollX,posY_g=rect_g.top+window.scrollY,width_c=rect_g.width,height_c=rect_g.height;let x_g=(e.pageX-posX_g)/width_c*d3.boxplot.scale,y_g=d3.boxplot.scale-(e.pageY-posY_g)/height_c*d3.boxplot.scale;let match=d3.boxplot.mousetip.get_match(e);let x_zone="unknown";for(let zone in d3.boxplot.x_zones){if(d3.boxplot.x_zones[zone][0]<x_g&&x_g<=d3.boxplot.x_zones[zone][1]){x_zone=d3.boxplot.mousetip.get_label(zone);break}}let y_zone="unknown";for(let zone in d3.boxplot.y_zones){if(d3.boxplot.y_zones[zone][0]<y_g&&y_g<=d3.boxplot.y_zones[zone][1]){y_zone=d3.boxplot.mousetip.get_label(zone);break}}let html="";if(match!==null){html=`<strong>Query:</strong> ${match.y_zone}<br/>(${match.y_match[0]} - ${match.y_match[1]})<br/>
                        ...
                        </table>`}tip.html(html);let css={top:mouseY,left:mouseX};if(match===null){css.color="#000";css.background="#ededed"}else{let idy_class="";if(match.idy>=0.75){idy_class="3"}else if(match.idy>=0.5){idy_class="2"}else if(match.idy>=0.25){idy_class="1"}else if(match.idy>=0){idy_class="0"}else{idy_class="-1"}css.background=d3.boxplot.color_idy[d3.boxplot.color_idy_theme][idy_class];css.color=d3.boxplot.mousetip.getColorByBgColor(css.background)}if(!hidden&&(!e.ctrlKey&&!d3.boxplot.zone_selected||match!==null)){tip.show().css(css)}},0)})};/**
 * Initialise tooltip
 */d3.boxplot.mousetip.init=function(){$("#draw").append($("<span>").attr("class","tip"));$("g.container").mousetip(".tip","#draw")};/**
 * Hide tooltip
 ...
 * Click event action
 */d3.boxplot.zoom.click=function(){if(!d3.event.ctrlKey&&!d3.boxplot.all_disabled){let cursor=d3.boxplot.zoom._cursor_pos();d3.boxplot.select_zone(cursor[0],cursor[1])}};/**
 ...
 */d3.boxplot.zoom.reset_scale=function(temp=false,after=null,force=true){if(!d3.boxplot.all_disabled||force){dgenies.show_loading();window.setTimeout(()=>{//Reset scale:
