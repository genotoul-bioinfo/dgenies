...
d3.dgenies.draw_top_track();d3.dgenies.draw_right_track();d3.dgenies.zoom_enabled=false}$("#restore-all").show();dgenies.hide_loading()},0)};/**
 ...
 * Draw top track
 */d3.dgenies.draw_top_track=function(){$("svg.top-track").remove();//Remove previous track (if any)
let transform=d3.dgenies.container.attr("transform");if(transform===null){transform="translate(0,0)scale(1)"}let tr_regex=/translate\(([^,]+),([^)]+)\)/;let translate=parseFloat(transform.match(tr_regex)[1]);let sc_regex=/scale\(([^,)]+)(,([^)]+))?\)/;let scale=parseFloat(transform.match(sc_regex)[1]);let svg_top=d3.dgenies.svgsupercontainer.append("svg:svg").attr("class","track top-track").attr("width",d3.dgenies.container_dotplot_width-d3.dgenies.container_axis_thickness).attr("height",d3.dgenies.container_track_thickness).attr("x",d3.dgenies.container_axis_thickness).attr("y",d3.dgenies.container_axis_thickness).attr("viewBox","0 0 1000 1000").attr("preserveAspectRatio","none");svg_top.append("rect").attr("x",0).attr("y",0).attr("width","100%").attr("height","100%").attr("fill","#ffffff").attr("stroke","none");if(d3.dgenies.x_track!==null){const track_type=d3.dgenies.x_track.type;let x_track=d3.dgenies.x_track.data;for(let x_id in x_track){if(x_track!==undefined){for(let feat of x_track[x_id]){let f_start=feat[0];let f_len=feat[1];let f_rep=feat[2];let f_val=feat[3];let f_comment=feat[4];start=translate+scale*feat[0]/d3.dgenies.x_len*d3.dgenies.scale;length=scale*feat[1]/d3.dgenies.x_len*d3.dgenies.scale;let block=svg_top.append("rect").attr("x",start).attr("y",0).attr("width",length).attr("height","100%").attr("fill","#0000ff").attr("stroke","none");if(f_val!=""){block.attr("fill",f_val)}}}}}};/**
 * Draw right track
 */d3.dgenies.draw_right_track=function(){$("svg.right-track").remove();//Remove previous track (if any)
let transform=d3.dgenies.container.attr("transform");if(transform===null){transform="translate(0,0)scale(1)"}let tr_regex=/translate\(([^,]+),([^)]+)\)/;let translate=parseFloat(transform.match(tr_regex)[2]);let sc_regex=/scale\(([^,)]+)(,([^)]+))?\)/;let scale=parseFloat(transform.match(sc_regex)[1]);let y_max=d3.dgenies.y_len+translate/d3.dgenies.scale*d3.dgenies.y_len/scale;let y_min=y_max-d3.dgenies.y_len/scale;let svg_right=d3.dgenies.svgsupercontainer.append("svg:svg").attr("class","track right-track").attr("width",d3.dgenies.container_track_thickness).attr("height",d3.dgenies.container_dotplot_height-d3.dgenies.container_axis_thickness).attr("x",d3.dgenies.container_dotplot_width).attr("y",d3.dgenies.container_axis_thickness+d3.dgenies.container_track_thickness).attr("viewBox","0 0 1000 1000").attr("preserveAspectRatio","none");svg_right.append("rect").attr("x",0).attr("y",0).attr("width","100%").attr("height","100%").attr("fill","#ffffff");if(d3.dgenies.y_track!==null){const track_type=d3.dgenies.y_track.type;let y_track=d3.dgenies.y_track.data;for(let y_id in y_track){if(y_track!==undefined){for(let feat of y_track[y_id]){let f_start=feat[0];let f_len=feat[1];let f_rep=feat[2];let f_val=feat[3];let f_comment=feat[4];start=translate+scale*(d3.dgenies.y_len-feat[0]-feat[1])/d3.dgenies.y_len*d3.dgenies.scale;length=scale*feat[1]/d3.dgenies.y_len*d3.dgenies.scale;let block=svg_right.append("rect").attr("x",0).attr("y",start).attr("width","100%").attr("height",length).attr("fill","#0000ff").attr("stroke","none");if(f_val!=""){block.attr("fill",f_val)}}}}}};/**
...
d3.dgenies.draw_top_axis();d3.dgenies.draw_right_axis();d3.dgenies.zoom_bottom_axis();d3.dgenies.zoom_left_axis();//Update traxks:
d3.dgenies.draw_top_track();d3.dgenies.draw_right_track()}else if(d3.dgenies.translate_start!==null){let help_trans=$("#help-trans");help_trans.fadeIn("slow")}};/**
 * Get cursor position
 *
...
d3.dgenies.draw_top_axis();d3.dgenies.draw_right_axis();d3.dgenies.zoom_bottom_axis();d3.dgenies.zoom_left_axis();d3.dgenies.draw_top_track();d3.dgenies.draw_right_track();if(translate_x<=0.00001&&translate_x>=-0.00001&&translate_y<=0.00001&&translate_y>=-0.00001&&new_scale===1){$("#restore-all").hide()}else{$("#restore-all").show()}}}else if(d3.dgenies.zoom_enabled){let help_zoom=$("#help-zoom");help_zoom.fadeIn("slow");if(d3.dgenies.zoom.help_timeout!==null){window.clearTimeout(d3.dgenies.zoom.help_timeout)}d3.dgenies.zoom.help_timeout=window.setTimeout(()=>{help_zoom.fadeOut("slow")},700)}};/**
...
d3.dgenies.draw_right_track();d3.dgenies.draw_top_track();if(!temp)d3.dgenies.zone_selected=false;dgenies.hide_loading();//Re-enable zoom:
