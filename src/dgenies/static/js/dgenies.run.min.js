...
dgenies.run.s_id=null;dgenies.run.allowed_ext=[];dgenies.run.max_upload_file_size=-1;dgenies.run.files=[undefined,undefined,undefined,undefined,undefined,undefined,undefined];dgenies.run.files_nb={"query":0,"target":1,"queryidx":2,"targetidx":3,"alignfile":4,"backup":5,"batch":6};dgenies.run.allow_upload=false;dgenies.run.ping_interval=null;dgenies.run.target_example="";dgenies.run.query_example="";dgenies.run.backup_example="";dgenies.run.batch_example="";dgenies.run.tool_has_ava={};dgenies.run.max_jobs=1;dgenies.run.enabled=true;dgenies.run.valid=true;// Keys in batch file that will use files
dgenies.run.KEYS_FOR_FILES=["alignfile","backup","query","target"];//dgenies.run.FILE_STATES = ["available", "duplicated", "missing", "unused"]
 ...
 */dgenies.run.init=function(s_id,allowed_ext,max_upload_file_size=1073741824,target_example="",query_example="",backup_example="",batch_example="",tool_has_ava={},max_jobs=1){dgenies.run.s_id=s_id;dgenies.run.allowed_ext=allowed_ext;dgenies.run.max_upload_file_size=max_upload_file_size;dgenies.run.target_example=target_example;dgenies.run.query_example=query_example;dgenies.run.batch_example=batch_example;dgenies.run.backup_example=backup_example;dgenies.run.tool_has_ava=tool_has_ava;dgenies.run.max_jobs=max_jobs;dgenies.run.restore_form();dgenies.run.set_events();dgenies.run.init_fileuploads()};/**
...
$(":button[id^='delete-btn']").click(function(){let i=$(this).parents("tr").data("index");dgenies.run.files_for_batch.splice(i,1);dgenies.run.check_files();dgenies.run.refresh_listing()})};/**
 ...
 **/dgenies.run.check_files=function(){let file_list=Array.from(dgenies.run.files_for_batch,elem=>elem.files[0].name);dgenies.run.file_states=dgenies.run.check_uploaded_files(dgenies.run.files_in_batch,file_list);dgenies.run.missing_files=dgenies.run.check_missing_files(dgenies.run.files_in_batch,file_list)};/**
 * Generate the list of filenames used in the list of jobs
 * @param {array} job_list list of jobs
 * @returns {array} a list of string
 **/dgenies.run.get_local_files=function(job_list){let needed_files=new Set;for(let j of job_list){for(let k of dgenies.run.KEYS_FOR_FILES){// We check if file isn't an url
if(`${k}_type`in j&&j[`${k}_type`]==="local"){needed_files.add(j[k])}}}return Array.from(needed_files)};/**
 ...
 **/dgenies.run.line_to_job=function(str){let result={};let array=str.trim().split(/[ \t]+/).map(// each line is transformed in list of key=value
kv=>kv.match(/\b(?<key>[^ \t=]+)=(?<value>[^ \t]+)\b/));// We extract the key: value from matches.
for(let e of array){if(e!==null){result[e.groups.key]=e.groups.value}}// adjust align to alignfile key used as entry
if("align"in result){result["alignfile"]=result["align"];delete result["align"]}// add the file type (url or local)
for(let k of dgenies.run.KEYS_FOR_FILES){if(`${k}`in result){if(dgenies.run.check_url(result[`${k}`])){result[`${k}_type`]="url"}else{result[`${k}_type`]="local"}}}return result};/**
 * Parse a batch file and add it in html
 **/dgenies.run.read_batch=function(){console.log("Change batch file");const[f]=this.files;const reader=new FileReader;// resulting job list
if(f){reader.onload=function(e){// We normalize seperators and blank lines
let job_array=e.target.result.trim().replace(/[ \t]+/g,"\t").split(/[\n\r]+/);// We limit the number of jobs
if(job_array.length>dgenies.run.max_jobs){dgenies.notify(`Batch file too long, only ${dgenies.run.max_jobs} first jobs were considered!`,"danger",3000);job_array=job_array.slice(0,dgenies.run.max_jobs)}// We fill the html field with processed file content
$("#batch_content").text(job_array.join("\n"));// We generate job list from job_array
dgenies.run.job_list=job_array.map(dgenies.run.line_to_job).filter(obj=>!(obj&&Object.keys(obj).length===0&&obj.constructor===Object));dgenies.run.files_in_batch=dgenies.run.get_local_files(dgenies.run.job_list);dgenies.run.check_files();dgenies.run.refresh_listing()};reader.onerror=function(e){$("#batch_content").text(e.target.error);console.log(e.target.error)};reader.readAsText(f)}else{$("#batch_content").text("No batch file loaded")}};/**
 * Restore run form
 ...
 */dgenies.run.upload_next=function(){let next=dgenies.run.files_to_upload.pop();console.log(next);while(next===undefined&&dgenies.run.files_to_upload.length>0){next=dgenies.run.files_to_upload.pop()}if(next!==undefined){next.submit();return true}dgenies.run.do_submit();return false};/**
...
$(":button[id='delete-unused-btn']").click(function(){dgenies.run.jq_remove_from_listing($("#listing").find("tr.unused-file"))});// We set behavior of 'clear all' button
$(":button[id='delete-all-btn']").click(function(){dgenies.run.files_for_batch=new Array;dgenies.run.refresh_listing()});// We set bname behavior on change
 ...
 */dgenies.run.show_tab=function(tab){$(`#tabs #${tab}`).addClass("active");$(`#tabs .tab:not(#${tab})`).removeClass("active");$(`.tabx:not(${tab})`).hide();$(`.tabx.${tab}`).show()};/**
...
if($("input#bname").val().length===0){$("label.file-batch").addClass("error");dgenies.run.add_error("Batch file is required!");has_errors=true}// Check that no file in batch file is missing
 ...
 * @returns {boolean} true if valid, else false
 */dgenies.run.check_url=function(url){return url.startsWith("http://")||url.startsWith("https://")||url.startsWith("ftp://")||url.startsWith("example://")};/**
 * Check upload stuff
 *
