 ...
 */dgenies.run.parse_batch=function(text){// We normalize seperators and blank lines
let job_array=text.trim().replace(/[ \t]+/g,"\t").split(/[\n\r]+/);// We limit the number of jobs
...
dgenies.run.job_list=job_array.map(dgenies.run.line_to_job).filter(obj=>!(obj&&Object.keys(obj).length===0&&obj.constructor===Object));dgenies.run.files_in_batch=dgenies.run.get_local_files(dgenies.run.job_list);dgenies.run.check_files();dgenies.run.refresh_listing()};/**
...
if(f){reader.onload=function(e){dgenies.run.parse_batch(e.target.result)};reader.onerror=function(e){$("#batch_content").text(e.target.error);console.log(e.target.error)};reader.readAsText(f)}else{$("#batch_content").text("No batch file loaded")}};/**
 * Restore run form
 ...
 */dgenies.run.fill_examples=function(tab){dgenies.run.show_tab(tab);if(tab=="tab1"){$("select.target").val("1").trigger("change");$("input#target").val("example://"+dgenies.run.target_example);if(dgenies.run.query_example!==""){$("select.query").val("1").trigger("change");$("input#query").val("example://"+dgenies.run.query_example)}}if(tab=="tab2"){$("select.backup").val("1").trigger("change");$("input#backup").val("example://"+dgenies.run.backup_example)}if(tab=="tab3"){$("select.batch").val("1").trigger("change");/* TODO:
         ...
         */dgenies.get("/example/batch",{},function(data){$("#bname").val("");dgenies.run.parse_batch(data)})}};/**
