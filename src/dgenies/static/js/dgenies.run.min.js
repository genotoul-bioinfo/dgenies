 ...
 */dgenies.run.check_file_format_and_presence=function(type,key,val){let corrected_key=key;if((key=="query"||key=="target")&&type=="plot"){corrected_key=key+"idx"}else if(key=="align"){corrected_key="alignfile"}if(dgenies.run.KEYS_FOR_FILES.includes(corrected_key)&&!dgenies.run.check_url(val,false)){if(val.startsWith("example://")){let example_urls=[];if(`${key}_example`in dgenies.run){example_urls.push("example://"+dgenies.run[`${key}_example`])}if(key==="query"&&dgenies.run.target_example!==undefined){example_urls.push("example://"+dgenies.run.target_example)}else if(key==="target"&&dgenies.run.query_example!==undefined){example_urls.push("example://"+dgenies.run.query_example)}if(!example_urls.includes(val)){return{message:example_urls===[]?`No example url for "${key}"`:`Example link must be: "${example_urls.join("\" or \"")}"`,severity:"error"}}}else{let regexps=dgenies.run.FTYPES[corrected_key].formats.map(function(x){return dgenies.run.FTYPES_REGEX[x]});let has_match=false;for(let e of regexps){has_match=has_match||e.test(val)}if(!has_match){return{message:`File must be have the following format for "${key}": ${dgenies.run.FTYPES[corrected_key].formats.join(", ")}`,severity:"error"}}else{let uploaded_files=dgenies.run.get_uploaded_files();if(!uploaded_files.includes(val)){return{message:`Missing file in user files: ${val}`,severity:"error"}}}}}return undefined};/**
 * Check if file has the right extension.
 * 
