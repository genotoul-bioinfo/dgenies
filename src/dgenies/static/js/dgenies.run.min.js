 ...
 */dgenies.run.init=function(s_id,allowed_ext,max_upload_file_size=1073741824,target_example="",query_example="",backup_example="",batch_example="",tool_has_ava={}){dgenies.run.s_id=s_id;dgenies.run.allowed_ext=allowed_ext;dgenies.run.max_upload_file_size=max_upload_file_size;dgenies.run.target_example=target_example;dgenies.run.query_example=query_example;dgenies.run.batch_example=batch_example;dgenies.run.backup_example=backup_example;dgenies.run.tool_has_ava=tool_has_ava;dgenies.run.restore_form();dgenies.run.set_events();dgenies.run.init_fileuploads()};/**
 * Restore run form
 ...
 */dgenies.run.upload_next=function(){let next=dgenies.run.files.pop();while(next===undefined&&dgenies.run.files.length>0){next=dgenies.run.files.pop()}if(next!==undefined){next.submit();return true}dgenies.run.do_submit();return false};/**
...
$(`input#${ftype}`).val(data["files"][0]["name"]);dgenies.run.hide_loading(ftype);dgenies.run.show_success(ftype);dgenies.run.upload_next()}},error:function(data,success){dgenies.run.__upload_server_error(ftype,data)}})};/**
...
$(`button#button-${ftype}`).click(function(){$(`input.file-${ftype}`).trigger("click")})})};/**
...
if($("input#batch").val().length===0){$("label.file-batch").addClass("error");dgenies.run.add_error("Batch file is required!");has_errors=true}}// Returns
return!has_errors};/**
