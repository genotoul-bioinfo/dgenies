 ...
 * Restore run form
 */dgenies.run.restore_form=function(){let ftypes=["query","target","queryidx","targetidx","alignfile","backup"];for(let f in ftypes){let ftype=ftypes[f];dgenies.run.change_fasta_type(ftype,$(`select.${ftype}`).find(":selected").text().toLowerCase(),true)}};/**
 * Upload next file
 *
 ...
 */dgenies.run.init_fileuploads=function(){let ftypes={"query":{"formats":["fasta"]},"target":{"formats":["fasta"]},"queryidx":{"formats":["fasta","idx"]},"targetidx":{"formats":["fasta","idx"]},"alignfile":{"formats":["map"]},"backup":{"formats":["backup"]}};$.each(ftypes,function(ftype,data){let formats=data["formats"];let position=dgenies.run.files_nb[ftype];dgenies.run._init_fileupload(ftype,formats,position);//Trigger events on hidden file inputs:
 ...
 */dgenies.run.show_tool_options=function(tool){$(`.optionx:not(tool-options-${tool})`).addClass("hidden");if($(`.optionx.tool-options-${tool}`).length>1){$(`.optionx.tool-options-${tool}`).removeClass("hidden")}};dgenies.run.change_slider_text=function(x){//console.log($(`${x}`))
//console.log($(`${x}-text`)[0].textContent)
$(`${x}-text`)[0].textContent=dgenies.run.tool_options[x][$(`${x}`).val()]};/**
 * Initialise events
 */dgenies.run.set_events=function(){let ftypes=["query","target","alignfile","queryidx","targetidx","backup"];$.each(ftypes,function(i,ftype){dgenies.run._set_file_event(ftype);dgenies.run._set_file_select_event(ftype)});$("button#submit").click(function(){dgenies.run.submit()});$("button#example").click(function(){dgenies.run.fill_examples()});$("#tabs .tab").click(function(){dgenies.run.show_tab($(this).attr("id"))});$("input[name=tool]").click(function(){dgenies.run.show_tool_options($(this).val())})};/**
 ...
 * Enable run form
 */dgenies.run.enable_form=function(){$(".progress").find(".bar").css("width","0%");$("input, select, button").prop("disabled",false);$("div#uploading-loading").hide();$("button#submit").show();let ftypes=["query","target","targetidx","queryidx","alignfile","backup"];for(let f in ftypes){let ftype=ftypes[f];dgenies.run.hide_loading(ftype);dgenies.run.hide_success(ftype)}dgenies.run.files=[undefined,undefined,undefined,undefined,undefined,undefined];dgenies.run.restore_form();dgenies.run.enabled=true};/**
 * Reset file input
 *
