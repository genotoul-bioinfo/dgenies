...
dgenies.run.s_id=null;dgenies.run.allowed_ext=[];dgenies.run.max_upload_file_size=-1;dgenies.run.files=[undefined,undefined,undefined,undefined,undefined,undefined,undefined,undefined];dgenies.run.files_nb={"query":0,"target":1,"queryidx":2,"targetidx":3,"alignfile":4,"backup":5,"querytrack":6,"targettrack":7};dgenies.run.allow_upload=false;dgenies.run.ping_interval=null;dgenies.run.target_example="";dgenies.run.query_example="";dgenies.run.backup_example="";dgenies.run.batch_example="";dgenies.run.tools=[];dgenies.run.tool_has_ava={};dgenies.run.tools_checking={};dgenies.run.tools_options={};dgenies.run.max_jobs=1;dgenies.run.enabled=true;dgenies.run.valid=true;dgenies.run.FTYPES={"query":{"formats":["fasta"]},"target":{"formats":["fasta"]},"queryidx":{"formats":["fasta","idx"]},"targetidx":{"formats":["fasta","idx"]},"alignfile":{"formats":["map"]},"backup":{"formats":["backup"]},"batch":{"formats":["batch"]},"querytrack":{"formats":["track"]},"targettrack":{"formats":["track"]}};dgenies.run.FTYPES_REGEX={"fasta":/^.+\.(fa|fna|fasta)(\.gz)?$/,"idx":/^.+\.idx(\.gz)?$/,"map":/^.+\.paf$/,"backup":/^.+\.tar(\.gz)?$/,"batch":/^.+\.(tab|tsv|txt)$/,"track":/^.+\.(bed|wig)$/};// Keys in batch file that will use files
 ...
 * Restore run form
 */dgenies.run.restore_form=function(){let ftypes=["query","target","queryidx","targetidx","alignfile","backup","querytrack","targettrack"];for(let f in ftypes){let ftype=ftypes[f];dgenies.run.change_fasta_type(ftype,$(`select.${ftype}`).find(":selected").text().toLowerCase(),true)}if(dgenies.run.editor!==null){dgenies.run.editor.setOption("readOnly",false)}};/**
 * Upload next file
 *
 ...
 * Disable run form
 */dgenies.run.disable_form=function(){dgenies.run.enabled=false;let ftypes=["query","target","targetidx","queryidx","alignfile","backup","querytrack","targettrack","localfiles"];for(let ftype of ftypes){dgenies.run.disable_dropzone(ftype)}$("input, select, button").prop("disabled",true);if(dgenies.run.editor!==null){dgenies.run.editor.setOption("readOnly",true)}};/**
 * Enable run form
 */dgenies.run.enable_form=function(){$(".progress").find(".bar").css("width","0%");$("input, select, button").prop("disabled",false);$("div#uploading-loading").hide();$("button#submit").show();let ftypes=["query","target","targetidx","queryidx","alignfile","backup","querytrack","targettrack","localfiles"];for(let ftype of ftypes){dgenies.run.hide_loading(ftype);dgenies.run.hide_success(ftype);dgenies.run.enable_dropzone(ftype)}dgenies.run.files=[undefined,undefined,undefined,undefined,undefined,undefined];dgenies.run.restore_form();dgenies.run.enabled=true};/**
 * Reset file input
 *
...
input_type="backup";input_file=$(`input#${input_type}`).val();if(input_file.length!==0){dgenies.run.reset_file_form("tab2",true);if(input_file.startsWith("example://")){let res=dgenies.run.check_example(input_type,input_file);if(!res.valid){has_errors=true;$(`label.file-${input_type}`).addClass("error");dgenies.run.add_error(`Backup file: ${res.message}`)}}}else{input_type="targetidx";input_file=$(`input#${input_type}`).val();if(input_file.length===0){$(`label.file-${input_type}`).addClass("error");dgenies.run.add_error("Target file is required!");has_errors=true}else if(input_file.startsWith("example://")){let res=dgenies.run.check_example(input_type,input_file);if(!res.valid){has_errors=true;$(`label.file-${input_type}`).addClass("error");dgenies.run.add_error(`Target file: ${res.message}`)}}input_type="queryidx";input_file=$(`input#${input_type}`).val();if(input_file.startsWith("example://")){let res=dgenies.run.check_example(input_type,input_file);if(!res.valid){has_errors=true;$(`label.file-${input_type}`).addClass("error");dgenies.run.add_error(`Query file: ${res.message}`)}}input_type="querytrack";input_file=$(`input#${input_type}`).val();if(input_file.startsWith("example://")){let res=dgenies.run.check_example(input_type,input_file);if(!res.valid){has_errors=true;$(`label.file-${input_type}`).addClass("error");dgenies.run.add_error(`Query track file: ${res.message}`)}}input_type="targettrack";input_file=$(`input#${input_type}`).val();if(input_file.startsWith("example://")){let res=dgenies.run.check_example(input_type,input_file);if(!res.valid){has_errors=true;$(`label.file-${input_type}`).addClass("error");dgenies.run.add_error(`Target track file: ${res.message}`)}}input_type="alignfile";input_file=$(`input#${input_type}`).val();if(input_file.length===0){$(`label.file-align`).addClass("error");dgenies.run.add_error("Alignment file is required!");has_errors=true}else if(input_file.startsWith("example://")){let res=dgenies.run.check_example(input_type,input_file);if(!res.valid){has_errors=true;$(`label.file-align`).addClass("error");dgenies.run.add_error(`Alignment file: ${res.message}`)}}}}/* TAB 3 */else{//Check input target:
 ...
 * @private
 */dgenies.run._has_upload=function(ftype,fname,is_multiple=false){let has_uploads=false;if(!is_multiple){let fasta_type=parseInt($(`select.${ftype}`).val());let fasta_val=$(`input#${ftype}`).val();if(fasta_type===0&&fasta_val.length>0){$(`button#button-${ftype}`).hide();dgenies.run.show_loading(ftype);has_uploads=true}else{dgenies.run.files[dgenies.run.files_nb[ftype]]=undefined;if(fasta_val!==""&&fasta_val!==undefined&&!dgenies.run.check_url(fasta_val)){dgenies.run.add_error(`${fname} file: invalid URL`,"error");dgenies.run.enable_form();return false}}}else{// missing and duplicated files were checked before
