 ...
 */dgenies.run.init=function(s_id,allowed_ext,max_upload_file_size=1073741824,target_example="",query_example="",tool_has_ava={}){dgenies.run.s_id=s_id;dgenies.run.allowed_ext=allowed_ext;dgenies.run.max_upload_file_size=max_upload_file_size;dgenies.run.target_example=target_example;dgenies.run.query_example=query_example;dgenies.run.tool_has_ava=tool_has_ava;dgenies.run.restore_form();dgenies.run.set_events();dgenies.run.init_fileuploads()};/**
 * Restore run form
 ...
 */dgenies.run.fill_examples=function(){dgenies.run.show_tab("tab1");$("select.target").val("1").trigger("change");$("input#target").val("example://"+dgenies.run.target_example);if(dgenies.run.query_example!==""){$("select.query").val("1").trigger("change");$("input#query").val("example://"+dgenies.run.query_example)}};/**
 ...
 * Initialise events
 */dgenies.run.set_events=function(){let ftypes=["query","target","alignfile","queryidx","targetidx","backup","batch"];$.each(ftypes,function(i,ftype){dgenies.run._set_file_event(ftype);dgenies.run._set_file_select_event(ftype)});$("button#submit").click(function(){dgenies.run.submit()});$("button#example").click(function(){dgenies.run.fill_examples()});$("#tabs .tab").click(function(){dgenies.run.show_tab($(this).attr("id"))});$("input[name=tool]").click(function(){dgenies.run.show_tool_options($(this).val())})};/**
