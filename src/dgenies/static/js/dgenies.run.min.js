...
let error=dgenies.run.check_file_format_and_presence(jobtype,key.image,val.image);if(error!==undefined){found.push({"message":error.message,"severity":error.severity,"from":CodeMirror.Pos(val.startLine-1,val.startColumn-1),"to":CodeMirror.Pos(val.endLine-1,val.endColumn)})}}return found};/**
