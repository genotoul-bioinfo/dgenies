 ...
 * Restore run form
 */dgenies.run.restore_form=function(){let ftypes=["query","target","alignfile","queryidx","targetidx","backup"];for(let f in ftypes){let ftype=ftypes[f];dgenies.run.change_fasta_type(ftype,$(`select.${ftype}`).find(":selected").text().toLowerCase(),true)}};/**
 * Upload next file
 *
 ...
 * Enable run form
 */dgenies.run.enable_form=function(){$(".progress").find(".bar").css("width","0%");$("input, select, button").prop("disabled",false);$("div#uploading-loading").hide();$("button#submit").show();let ftypes=["query","target","alignfile","targetidx","queryidx","backup"];for(let f in ftypes){let ftype=ftypes[f];dgenies.run.hide_loading(ftype);dgenies.run.hide_success(ftype)}dgenies.run.files=[undefined,undefined,undefined,undefined,undefined];dgenies.run.restore_form();dgenies.run.enabled=true};/**
 * Reset file input
 *
