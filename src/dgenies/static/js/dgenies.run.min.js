...
dgenies.run.files_for_batch=new Array;// states for files: dict of str: array of strings
dgenies.run.file_states=[];dgenies.run.missing_files=[];// Job list that will be send to server
dgenies.run.job_list=[];// list of fileupload that will be uploaded
...
let new_file_list=new Array;for(let i=0;i<dgenies.run.files_for_batch.length;i++){if(!selection_idx.includes(i)){new_file_list.push(dgenies.run.files_for_batch[i])}}dgenies.run.files_for_batch=new_file_list;// We refresh the listing
dgenies.run.check_files();dgenies.run.refresh_listing()};/**
 * Regenerate the html file listing
 */dgenies.run.refresh_listing=function(){let list=[];for(let i=0;i<dgenies.run.files_for_batch.length;i++){let data={"name":dgenies.run.files_for_batch[i].files[0].name,"size":dgenies.run.files_for_batch[i].files[0].size,"state":dgenies.run.file_states[i]};list.push(data)}// We update the html part
$("#listing").find("tbody:first").html(tmpl("tmpl-listing",list));// We reset the delete button events
$(":button[id^='delete-btn']").click(function(){let i=$(this).parents("tr").data("index");dgenies.run.files_for_batch.splice(i,1);dgenies.run.check_files();dgenies.run.refresh_listing();dgenies.run.relint()})};/**
 ...
 **/dgenies.run.check_uploaded_files=function(needed_files,uploaded_files){// list of states (following the order of list of files)
let states=[];// We count filename occurences
let count=new Map;for(let f of uploaded_files.flat()){if(count.has(f)){count.set(f,count.get(f)+1)}else{count.set(f,1)}}// We only check duplication for available files
for(let f of uploaded_files.flat()){if(needed_files.includes(f)){if(count.get(f)>1){states.push("duplicated")}else{states.push("available")}}else{states.push("unused")}}return states};/**
 ...
 * @param {array} uploaded_files list of filenames in upload field
 * @returns {array} a list of string with the same length than @uploaded_files
 **/dgenies.run.check_missing_files=function(needed_files,uploaded_files){return needed_files.filter(function(x){return uploaded_files.indexOf(x)<0})};/**
 * Compute states of files
 **/dgenies.run.check_files=function(){let file_list=dgenies.run.get_uploaded_files();dgenies.run.file_states=dgenies.run.check_uploaded_files(dgenies.run.files_in_batch,file_list);dgenies.run.missing_files=dgenies.run.check_missing_files(dgenies.run.files_in_batch,file_list)};/**
...
let mandatory=["target","type","tool"];let optional=["id_job","options"];if(has_ava){mandatory.push("query")}else{optional.push("query")}let all_allowed=mandatory.concat(optional);for(let param of job){let key=param[0];if(!all_allowed.includes(key.image)){found.push({"message":`Unknown key "${key.image}"`,"severity":"error","from":CodeMirror.Pos(key.startLine-1,key.startColumn-1),"to":CodeMirror.Pos(key.endLine-1,key.endColumn)})}}}else{// jobtype == "plot"
...
for(let k of dgenies.run.KEYS_FOR_FILES){if(`${k}`in job){if(dgenies.run.check_url(job[`${k}`])){job[`${k}_type`]="url"}else{job[`${k}_type`]="local"}}}}};/**
 ...
 * Restore run form
 */dgenies.run.restore_form=function(){let ftypes=["query","target","queryidx","targetidx","alignfile","backup","batch"];for(let f in ftypes){let ftype=ftypes[f];dgenies.run.change_fasta_type(ftype,$(`select.${ftype}`).find(":selected").text().toLowerCase(),true)}};/**
 * Upload next file
 *
...
$("#batch-multiple-files-upload").fileupload({dataType:"json",formData:{"s_id":dgenies.run.s_id,"formats":formats},maxNumberOfFiles:1,// max_files_for_a_job * number_of_jobs * 1.1 = 3 * 10 * 1.1
dropZone:$("#input-dropzone"),add:function(e,data){// We add the file
dgenies.run.files_for_batch.push(data);// We check the against the batch file
dgenies.run.check_files();// We refresh the file listing
dgenies.run.refresh_listing();// We relint the batch content
dgenies.run.relint()},drop:function(e,data){$.each(data.files,function(index,file){console.log("Dropped file: "+file.name)})},change:function(e,data){$.each(data.files,function(index,file){console.log("Selected file: "+file.name)})},done:function(e,data){console.log("Upload finished.")},success:function(data,success){console.log(data);if(data["success"]!=="OK"){dgenies.notify("message"in data?data["message"]:`An error has occured when uploading multiple file. Please contact us to report the bug!`,"danger");dgenies.run.enable_form()}else if("error"in data["files"][0]){dgenies.run.add_error("Query file: "+data["files"][0]["error"],"error");dgenies.run.enable_form()}else{dgenies.run.upload_next()}},error:function(data,success){dgenies.notify("message"in data?data["message"]:`An error has occured when uploading multiple files. Please contact us to report the bug!`,"danger");dgenies.run.enable_form()}})};/**
...
dgenies.run._init_multiple_fileupload(["fasta","idx","map","backup"]);$(":button[id='multiple-files-btn']").click(function(){$("#batch-multiple-files-upload").trigger("click")});// We set behavior of 'remove unused' button
$(":button[id='delete-unused-btn']").click(function(){dgenies.run.jq_remove_from_listing($("#listing").find("tr.unused-file"));dgenies.run.relint()});// We set behavior of 'clear all' button
$(":button[id='delete-all-btn']").click(function(){dgenies.run.files_for_batch=new Array;dgenies.run.refresh_listing();dgenies.run.relint()});// We set bname behavior on change
...
for(let j of jobs){found=found.concat(dgenies.run.ckeck_job(j))}// Refresh file listing, get local files, set a warning on those that are missing (needs the coordinates and look for a way to lint again when file list updated).
console.log(found);// TODO: remove get_local_files and get local file in dgenies.run.ckeck_job 
dgenies.run.files_in_batch=dgenies.run.get_local_files(dgenies.run.job_list);dgenies.run.check_files();dgenies.run.refresh_listing();// convert to POST message
dgenies.run.job_list=dgenies.run.create_job_list(jobs);//dgenies.run.adjust_job_list(jobs)
console.log(dgenies.run.job_list);return found});CodeMirror.defineMIME("text/plain","batch");dgenies.run.editor=CodeMirror.fromTextArea(batch_content,{lineNumbers:true,mode:"batch",lineWrapping:true,theme:"dgenies",gutters:["CodeMirror-lint-markers"],lint:true,selfContain:true})};/**
 ...
 */dgenies.run.show_tab=function(tab){$(`#tabs #${tab}`).addClass("active");$(`#tabs .tab:not(#${tab})`).removeClass("active");$(`.tabx:not(${tab})`).hide();$(`.tabx.${tab}`).show();if(tab=="tab3"){//activate timer
}else{//deactivate timer
}};/**
 * Change displayed options (if any)
 *
 * @param {string} tool id to show
 */dgenies.run.show_tool_options=function(tool){$(`.optionx:not(tool-options-${tool})`).addClass("hidden");if($(`.optionx.tool-options-${tool}`).length>1){$(`.optionx.tool-options-${tool}`).removeClass("hidden")}};/**
 * Initialise events
 */dgenies.run.set_events=function(){let ftypes=["query","target","alignfile","queryidx","targetidx","backup","batch"];$.each(ftypes,function(i,ftype){dgenies.run._set_file_event(ftype);dgenies.run._set_file_select_event(ftype)});$("button#submit").click(function(){dgenies.run.submit()});$("button#example_align").click(function(){dgenies.run.fill_examples("tab1")});$("button#example_backup").click(function(){dgenies.run.fill_examples("tab2")});$("button#dl_backup").click(function(){window.location="/example/backup"});$("button#example_batch").click(function(){dgenies.run.fill_examples("tab3")});$("button#view_batch").click(function(){$(this).find("a").click()});$("button#view_batch").find("a").click(function(){$(this).modal({showClose:false});return false});$("button#dl_batch").click(function(){window.location="/example/batch"});$("#tabs .tab").click(function(){dgenies.run.show_tab($(this).attr("id"))});$("input[name=tool]").click(function(){dgenies.run.show_tool_options($(this).val())})};/**
 ...
 * Disable run form
 */dgenies.run.disable_form=function(){dgenies.run.enabled=false;$("input, select, button").prop("disabled",true)};/**
 * Enable run form
 */dgenies.run.enable_form=function(){$(".progress").find(".bar").css("width","0%");$("input, select, button").prop("disabled",false);$("div#uploading-loading").hide();$("button#submit").show();let ftypes=["query","target","targetidx","queryidx","alignfile","backup","batch"];for(let f in ftypes){let ftype=ftypes[f];dgenies.run.hide_loading(ftype);dgenies.run.hide_success(ftype)}dgenies.run.files=[undefined,undefined,undefined,undefined,undefined,undefined,undefined];dgenies.run.restore_form();dgenies.run.enabled=true};/**
 * Reset file input
 *
 ...
 */dgenies.run.reset_file_form=function(tab,except_backup=false){let ftypes=[];let i=0;if("tab2"===tab){ftypes=["alignfile","queryidx","targetidx"];if(!except_backup){ftypes.push("backup")}}else if("tab3"===tab){ftypes=["batch"]}else{ftypes=["query","target"]}$.each(ftypes,function(i,ftype){dgenies.run.reset_file_input(ftype)})};/**
...
let tool=$("input[name=tool]:checked").val();if(!dgenies.run.tool_has_ava[tool]&&$("input#query").val().length===0){$("label.file-query").addClass("error");dgenies.run.add_error("Query fasta is required!");has_errors=true}}/* TAB 2 */else if(tab==="tab2"){if($("input#backup").val().length!==0){dgenies.run.reset_file_form("tab2",true)}else{if($("input#targetidx").val().length===0){$("label.file-targetidx").addClass("error");dgenies.run.add_error("Target file is required!");has_errors=true}if($("input#alignfile").val().length===0){$("label.file-align").addClass("error");dgenies.run.add_error("Alignment file is required!");has_errors=true}}}/* TAB 3 */else{//Check input target:
let inputText=dgenies.run.editor.getValue();if(inputText.length===0){$("label.file-batch").addClass("error");dgenies.run.add_error("Batch file is required!");has_errors=true}//Parse input target and check for errors:
if(inputText.length===0){$("label.file-batch").addClass("error");dgenies.run.add_error("Batch file is required!");has_errors=true}// Check that no file in batch file is missing
if(dgenies.run.missing_files.length>0){dgenies.run.add_error("Some input files in batch file are missing!");has_errors=true}// Check that no file in multiple upload field is duplicated
if(dgenies.run.file_states.some(element=>element==="duplicated")){dgenies.run.add_error("Some input files are duplicated!");has_errors=true}}// Returns
return!has_errors};/**
 * Show loading for a fasta uploading file
 *
 * @param {string} fasta uploading file type (query, target, ...)
 */dgenies.run.show_loading=function(fasta){$(".loading-file."+fasta).show()};/**
 * Hide loading for a fasta uploaded file
 *
 * @param {string} fasta uploaded file type (query, target, ...)
 */dgenies.run.hide_loading=function(fasta){$(".loading-file."+fasta).hide()};/**
 * Show success: file uploaded successfully
 *
 * @param {string} fasta uploaded type of file (query, target, ...)
 */dgenies.run.show_success=function(fasta){$(".upload-success."+fasta).show()};/**
 * Hide success on a file
 *
 * @param {string} fasta type of file (query, target, ...)
 */dgenies.run.hide_success=function(fasta){$(".upload-success."+fasta).hide()};/**
...
if(dgenies.run.file_states.some(element=>element==="available")){has_uploads=true;// TODO: hide/deactivate delete buttons?
...
inputs=[["batch-multiple-files-upload",`File listing`,true]];dgenies.run.files_to_upload=[];for(let i=0;i<dgenies.run.files_for_batch.length;i++){if(dgenies.run.file_states[i]==="available"){dgenies.run.files_to_upload.push(dgenies.run.files_for_batch[i])}}}// We check if there is some files to upload
...
dgenies.run.batchParser.Affectation=chevrotain.createToken({name:"Affectation",pattern:/=/,push_mode:"value_mode"});dgenies.run.batchParser.Value=chevrotain.createToken({name:"Value",pattern:/[^\s'"]+/,pop_mode:true});dgenies.run.batchParser.DQuotedValue=chevrotain.createToken({name:"DQuotedValue",pattern:/"[^"\r\n]+"/,pop_mode:true});dgenies.run.batchParser.SQuotedValue=chevrotain.createToken({name:"SQuotedValue",pattern:/'[^'\r\n]+'/,pop_mode:true});dgenies.run.batchParser.Key=chevrotain.createToken({name:"Key",pattern:/(type|align|query|target|backup|tool|options|id_job)/});dgenies.run.batchParser.NewLines=chevrotain.createToken({name:"NewLines",pattern:/[\n\r]+/});dgenies.run.batchParser.Spaces=chevrotain.createToken({name:"Spaces",pattern:/[\t ]+/});dgenies.run.batchParser.multiModeBatchLexerDefinition={modes:{key_mode:[dgenies.run.batchParser.Key,dgenies.run.batchParser.Affectation,dgenies.run.batchParser.Spaces,dgenies.run.batchParser.NewLines],value_mode:[dgenies.run.batchParser.DQuotedValue,dgenies.run.batchParser.SQuotedValue,dgenies.run.batchParser.Value,dgenies.run.batchParser.Spaces]},defaultMode:"key_mode"};// Our new lexer now support 3 different modes
