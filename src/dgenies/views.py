...
from dgenies.job_helpers import build_fasta, compute_summary, update_files
...
from dgenies.lib.exceptions import DGeniesExampleInvalid, DGeniesJobCheckError,\
...
def status(id_job):
    ...
    if job.is_batch():
        answer["batch"] = [Functions().get_status(JobManager(subjob_id)) for subjob_id in job.get_subjob_ids()]
...
def prepare_fasta(id_res: str):
    """
    Trigger and get status about the build of the fasta file of query
    ...
    try:
        status, is_compressed = build_fasta(id_res, to_compress)
        if status == 1:
            return jsonify({
                "success": True,
                "status": status,
...
def no_assoc(id_res):
    ...
    if os.path.exists(res_dir) and os.path.isdir(res_dir):
        ...
        try:
            ...
            print("Unable to load data!")
            abort(404)
        file_content = "\n".join(paf.build_list_no_assoc(request.form["to"])) + "\n"
        empty = file_content == "\n"
        return jsonify({
